...
Generate dataframe for all mean index high mass tanks
```{r}
burned.high.mass.df=DOC.df2[DOC.df2$Treatment=="burned",]

burned.high.mass.df$mean.SUVA=mean(DOC.df2[DOC.df2$Treatment=="burned",]$SUVA)
burned.high.mass.df$mean.sr=mean(DOC.df2[DOC.df2$Treatment=="burned",]$sr)
burned.high.mass.df$mean.HIX=mean(DOC.df2[DOC.df2$Treatment=="burned",]$hix)
burned.high.mass.df$mean.BIX=mean(DOC.df2[DOC.df2$Treatment=="burned",]$fr)
burned.high.mass.df$mean.FI=mean(DOC.df2[DOC.df2$Treatment=="burned",]$fl)

unburned.high.mass.df=DOC.df2[DOC.df2$Treatment=="unburned",]
unburned.high.mass.df %>% top_n(1, replace = FALSE)

unburned.high.mass.df$mean.SUVA=mean(DOC.df2[DOC.df2$Treatment=="unburned",]$SUVA)
unburned.high.mass.df$mean.sr=mean(DOC.df2[DOC.df2$Treatment=="unburned",]$sr)
unburned.high.mass.df$mean.HIX=mean(DOC.df2[DOC.df2$Treatment=="unburned",]$hix)
unburned.high.mass.df$mean.BIX=mean(DOC.df2[DOC.df2$Treatment=="unburned",]$fr)
unburned.high.mass.df$mean.FI=mean(DOC.df2[DOC.df2$Treatment=="unburned",]$fl)



mean.high.mass.index.df=data.frame(rbind(burned.high.mass.df, unburned.high.mass.df))

mean.high.mass.index.df=data.frame(mean.high.mass.index.df[mean.high.mass.index.df$Time.point!="T0",])

mean(mean.high.mass.index.df[mean.high.mass.index.df$Treatment=="unburned",]$fl)
...
New dataframe for high biomass indices
```{r}
DOC.df2=DOC.df[DOC.df$plant.mass..g>=200,]

index.d2=index.df[index.df$plant.mass..g.>=200,]

mean(DOC.df2[DOC.df2$Treatment=="unburned",]$hix)

```

Generate dataframe for high biomass tanks
```{r}
####Parametric output
fl.para.sums=as.data.frame(rbind(T1.fl.anova$pTerms.table, T2.fl.anova$pTerms.table, T3.fl.anova$pTerms.table, T4.fl.anova$pTerms.table))
names(fl.para.sums)[1]="df/edf"

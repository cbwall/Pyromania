...
m4.sage.gam=gam(perc.delta ~ treatment +s(plant.specific.mass..g, by= treatment), data=plant.df, method="REML", select= T, subset= plant.type=="sage")

hist(m3.sage.gam$residuals)

m5.sage.gam=gam(perc.delta ~ treatment +s(plant.specific.mass..g), data=plant.df, method="REML", select= T, subset= plant.type=="sage")

m6.sage.gam=gam(perc.delta ~ s(plant.specific.mass..g), data=plant.df, method="REML", select= T, subset= plant.type=="sage")

sage.AIC=AIC(m1.sage.gam, m2.sage.gam, m3.sage.gam, m4.sage.gam, m5.sage.gam, m6.sage.gam)
sage.AIC

sage.anova=anova(m3.sage.gam)
sage.anova
gam.check(m6.sage.gam)
...
m1.compare.burned.gam=gam(delta.trans ~ plant.type + s(plant.specific.mass..g, by=plant.type), data=plant.df, methods= "REML", select= T, subset= treatment == "burned")

m2.compare.burned.gam=gam(delta.trans ~ plant.type + s(plant.specific.mass..g), data=plant.df, methods= "REML", select= T, subset= treatment == "burned")

m3.compare.burned.gam=gam(delta.trans ~ s(plant.specific.mass..g), data=plant.df, methods= "REML", select= T, subset= treatment == "burned")
...
m1.compare.unburned.gam=gam(delta.trans ~ plant.type + s(plant.specific.mass..g, by=plant.type), data=plant.df, methods= "REML", select= T, subset= treatment == "unburned")

m2.compare.unburned.gam=gam(delta.trans ~ plant.type + s(plant.specific.mass..g), data=plant.df, methods= "REML", select= T, subset= treatment == "unburned")

m3.compare.unburned.gam=gam(delta.trans ~ s(plant.specific.mass..g), data=plant.df, methods= "REML", select= T, subset= treatment == "unburned")
...
compare.unburned.mod.plot<-
  ...
  geom_point(data=plant.df[(plant.df$treatment=="unburned"),], 
   ...
   scale_color_manual(values = c("mediumpurple4", "darkgoldenrod4")) +
     scale_fill_manual(values = c("mediumpurple4", "darkgoldenrod4")) +
  ggtitle("Unburned Sage vs. Willow") +
  ylab("Mass Loss (g)") +
  xlab("plant material (g)") +
  coord_cartesian(ylim = c(0,1))+
  Fig.formatting

...
m1.gam=gam(log(DOC..mg.L) ~ Treatment + s(plant.mass..g, k=8, m=1) + s(plant.mass..g, Time.point, k=8, bs="fs", m=1), data=DOC.df, method="REML")
summary.gam(m1.gam)
draw(m1.gam)
k.check(m1.gam)
...
```{r}
#log transformed
m2.gam=gam(log(DOC..mg.L) ~ Treatment + s(plant.mass..g, by=Treatment, k=8, m=1) + s(plant.mass..g, Time.point, k=8, bs="fs", m=1), data=DOC.df, method="REML")
summary(m2.gam)
draw(m2.gam)
```
...
```{r}
####Gamma distribution
m3.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, by=Treatment, k=8, m=1) + s(plant.mass..g, Time.point, k=8, bs="fs", m=1), family = Gamma(link = "log"), data=DOC.df, method="REML")
summary(m3.gam)
draw(m3.gam)
...
m4=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, by= Treatment, bs="ps", k=8, m=1), data= DOC_T4, method= "REML")
summary.gam(m4)
draw(m4)

...
abs.unburned.degra.plot=ggplot(degra.df[(degra.df$Treatment=="unburned"),])+ 
  ...
  geom_smooth(aes(x=plant.mass..g, y=abs.value.DOC, color=type), method="loess", alpha=0.1)+
  scale_color_manual(values = c("springgreen1", "gold", "midnightblue")) + 
  ggtitle("Degradation: unburned") +
  ylab(expression(paste(Delta, " DOC mg/L"))) +
...
uv.m1.gam=gam(abs.change.DOC ~ Treatment  + type + s(plant.mass..g, Time.point, bs="fs"), data=degra.df, method="REML")
summary(uv.m1.gam)

uv.m2.gam=gam(abs.value.DOC~ Treatment  + type + s(plant.mass..g, by=type, bs="fs") + s(plant.mass..g, Time.point, by=Treatment, bs="fs"), family= Gamma(link = "log"), data=degra.df, method="REML")
summary(uv.m2.gam)

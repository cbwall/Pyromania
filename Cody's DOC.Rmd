...
model 28 wins
```{r}
m12.gam=gam(DOC..mg.L ~ Treatment*Time.point  + s(plant.mass..g) + s(plant.mass..g, by=oTreatment) + s(plant.mass..g, by=oTime.point), family= Gamma(link = "log"), data=DOC.df2, select= T, method="REML")
summary(m12.gam)

m13.gam=gam(DOC..mg.L ~ Treatment*Time.point  + s(plant.mass..g, k=5) + s(plant.mass..g, by=oTreatment) + s(plant.mass..g, by=oTime.point), family= Gamma(link = "log"), data=DOC.df2, select= T, method="REML")
summary(m13.gam)

AIC(m12.gam, m13.gam)


m14.gam=gam(DOC..mg.L ~ Treatment*Time.point  + s(plant.mass..g, k=15) + s(plant.mass..g, by=oTreatment, k=4) + s(plant.mass..g, by=oTime.point, k=4), family= Gamma(link = "log"), data=DOC.df2, select= T, method="REML", drop.unused.levels = F)
summary(m14.gam)
concurvity(m14.gam)
...
m14.gam=gam(DOC..mg.L ~ Treatment*Time.point  + s(plant.mass..g) + s(plant.mass..g, by=oTreatment) + s(plant.mass..g, by=oTime.point), data=DOC.df2, select= T, method="REML")
summary(m14.gam)
concurvity(m14.gam)
gam.check(m14.gam, rep=1000)
...
m15.gam=gam(DOC..mg.L ~ Treatment*Time.point  + s(plant.mass..g) + s(plant.mass..g, by=oTreatment) + s(plant.mass..g, by=oTime.point), family= Gamma(link = "log"), data=DOC.df2, select= T, method="REML")
summary(m15.gam)
draw(m15.gam)
...
m16.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g) + s(plant.mass..g, by= oTreatment) + s(plant.mass..g, Time.point, bs="fs"), data=DOC.df2, method="REML")
summary(m16.gam)
gam.check(m16.gam, rep=1000)


m17.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, k=15) + s(plant.mass..g, by= oTreatment, k=4, m=1) + s(plant.mass..g, Time.point, bs="fs"), data=DOC.df2, family= Gamma(link = "log"), method="REML")
draw(m17.gam)
summary(m17.gam)
...
m18.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, k=15) + s(plant.mass..g, by= Treatment, k=4, m=1) + s(plant.mass..g, Time.point, bs="fs"), data=DOC.df2, family= Gamma(link = "log"), method="REML")
draw(m18.gam)
summary(m18.gam)

m19.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, k=15) + s(plant.mass..g, by= oTreatment, k=4, m=1) + s(plant.mass..g, Time.point, bs="fs"), data=DOC.df2, family= Gamma(link = "log"), method="REML")
summary(m19.gam)
draw(m19.gam)

m26.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, k=6) + s(plant.mass..g, Time.point, by= Treatment, k=4, bs="fs"), data=DOC.df2, select= T, family=Gamma(link = "log"), method="REML")
summary(m26.gam)
draw(m26.gam)

m27.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, k=6) + s(plant.mass..g, by= Treatment, k=4) + s(plant.mass..g, Time.point, bs="fs"), select= T, data=DOC.df2, family=Gamma(link = "log"), method="REML")
summary(m27.gam)
draw(m27.gam)
...
m28.gam=gam(DOC..mg.L ~ Treatment*Time.point + s(plant.mass..g, k=6) + s(plant.mass..g, by= Treatment, k=4) + s(plant.mass..g, by= Time.point), select= T, data=DOC.df2, family=Gamma(link = "log"), method="REML")
summary(m28.gam)
concurvity(m28.gam)


m29.gam=gam(DOC..mg.L ~ Treatment*Time.point + s(plant.mass..g, k=6) + s(plant.mass..g, by= oTreatment, k=4) + s(plant.mass..g, by= oTime.point), select= T, data=DOC.df2, family=Gamma(link = "log"), method="REML")
summary(m29.gam)
concrvity(m29.gam)

AIC(m26.gam, m27.gam, m28.gam, m29.gam)

ggplot(DOC.df2, aes(x=predict(m26.gam), y=DOC..mg.L), color=predict(m26.gam)) + 
  geom_point() +
  geom_smooth()+
  labs(x='Predicted Values', y='Actual Values', title='Predicted vs. Actual Values')

library(jtools)
effect_plot(m27.gam, pred = plant.mass..g, interval = TRUE, partial.residuals = TRUE)
...
```
DOC model 28 intepretation
```{r}
summary(m28.gam)$coefficient
...
vis.burn.plot=visreg(m28.gam, "plant.mass..g", ylab="DOC..mg.L", trans=exp, gg=TRUE, by="Time.point", overlay=T, partial=FALSE, rug=F, cond=list(Treatment="burned")) +
  geom_point(alpha=0.5) +
  ggtitle("Burned") +
  ylab("DOC (mg/L)")+
  xlab("plant mass (g)")+
...
vis.unburn.plot=visreg(m28.gam, "plant.mass..g", ylab="DOC..mg.L", gg=TRUE, trans=exp, by="Time.point", overlay=T, partial=FALSE, rug=F, cond=list(Treatment="unburned")) +
  geom_point(alpha=0.5) +
  ggtitle("Unburned") +
  ylab("DOC (mg/L)")+
  xlab("plant mass (g)")+
...
ggsave("figures/Fig.2.UVBio.mass.degra_type.plots.pdf", height=5, width=10)
...
uv.m1.gam=gam(mass.remaining.arc ~ Treatment*type + s(plant.mass..g) + s(plant.mass..g, by=Treatment) + s(plant.mass..g, by=type) +s(plant.mass..g, Time.point, bs="fs"), data=degra.df, method="REML", select= T)
summary(uv.m1.gam)
gam.check(uv.m1.gam, rep = 1000)
draw(uv.m1.gam)

uv.m2.gam=gam((mass.remaining)^1/2 ~ Treatment*type*Time.point + s(plant.mass..g) + s(plant.mass..g, by=oTreatment) + s(plant.mass..g, by=oTime.point) + s(plant.mass..g, by= otype), data=degra.df, method="REML", select= T)
summary(uv.m2.gam)
...
uv.m3.gam=gam(log10(mass.remaining) ~ Treatment*type*Time.point + s(plant.mass..g) + s(plant.mass..g, by=oTreatment) + s(plant.mass..g, by=oTime.point) + s(plant.mass..g, by= otype), family= gaussian, data=degra.df, method="REML", select= T)
summary(uv.m3.gam)
...
degra.plots
ggsave("figures/Fig.3.mass.decomp_type.plots.pdf", height=5, width=10)
...
ggsave("figures/fig4.suva.pdf", height=5, width=14)

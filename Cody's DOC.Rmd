...
degra.df$mass.remaining.dec=asin(sqrt(degra.df$mass.remaining.dec))

max(degra.df$mass.remaining.dec)
```
...
uv.m2.gam=gam(mass.remaining ~ Treatment*type*Time.point + s(plant.mass..g)+ s(plant.mass..g, by=Treatment) + s(plant.mass..g, by=Time.point) + s(plant.mass..g, by= type), data=degra.df, method="REML", select= T)
summary(uv.m2.gam)
...
uv.m3.gam=gam(mass.remaining ~ Treatment*type*Time.point + s(plant.mass..g, k=15)+ s(plant.mass..g, by=Treatment, k=15) + s(plant.mass..g, by=Time.point, k=15) + s(plant.mass..g, by= type, k=15), data=degra.df, method="REML", family=gaussian(link = "log"), select= T)
summary(uv.m3.gam)
gam.check(uv.m3.gam)
...
uv.m4.gam=gam(mass.remaining ~ Treatment*type*Time.point + s(plant.mass..g, k=15)+ s(plant.mass..g, by=Treatment, k=15, m=1) + s(plant.mass..g, by=Time.point, k=15, m=1) + s(plant.mass..g, by= type, k=15, m=1), data=degra.df, method="REML", select= T)
summary(uv.m4.gam) 
...
suva.t0.df=read.csv("/Users/tobyspiegel2/Desktop/My_pyromania/EEMS/SUVAresults/Results_T0.csv")
...
#load file and rearrange
library(dplyr)
suva.t1.df=read.csv("/Users/tobyspiegel2/Desktop/My_pyromania/EEMS/SUVAresults/Results_T1.csv")
...
```{r}
#load file and rearrange
suva.t2.df=read.csv("/Users/tobyspiegel2/Desktop/My_pyromania/EEMS/SUVAresults/Results_T2_d1A.edit.csv")
...
Time point 3
```{r}
suva.t3.df=read.csv("/Users/tobyspiegel2/Desktop/My_pyromania/EEMS/SUVAresults/Results_T3.csv")
...
Time point 4
```{r}
suva.t4.df=read.csv("/Users/tobyspiegel2/Desktop/My_pyromania/EEMS/SUVAresults/Results_T4.csv")
...
plot_grid(
  ggdraw(add_sub(suva.plots, "Fig 4. Average specific ultraviolet absorbance (SUVA 254 ) across sampling period for both treatments. Time-0 represents baseline DOC concentrations \nprior to the start of the experiment. ", x=0.01, hjust=0)), ncol = 1)

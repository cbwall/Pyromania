...
UVBIO.T1<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=plant.mass..g, y=percent.change.DOC, color=effect))+
  facet_grid(~Treatment)+
  geom_point(alpha=0.5) +
  scale_color_manual(values = c("midnightblue", "gold", "springgreen1")) + 
  geom_smooth(method=loess, aes(fill=effect), alpha=0.1) +
  ylab(expression(paste("%", Delta, "DOC (mg/L)")))+
  ggtitle("Time-1") +
  xlab("Plant Mass (g)")+
  theme_cowplot(12)
...
library(visreg)
uv.t1.burned=visreg(uv.m1.gam, "plant.mass..g", gg=TRUE, by="effect", overlay=T, partial=F, rug=F, cond=list(Treatment="burned")) +
  ggtitle("T1: Burned") +
  xlab("plant mass (g)")+
  theme_cowplot(12)

uv.t1.unburned=visreg(uv.m1.gam, "plant.mass..g", gg=TRUE, by="effect", overlay=T, partial=F, rug=F, cond=list(Treatment="unburned")) +
  ggtitle("T1: Unburned") +
  xlab("plant mass (g)")+
  theme_cowplot(12)

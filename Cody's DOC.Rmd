...
degra.df$mass.remaining.arc=asin(sqrt(degra.df$mass.remaining.dec/99.95216))
...
```{r}
#fit complex model 
uv.m1.gam=gam(mass.remaining ~ Treatment*type*Time.point + s(plant.mass..g) + s(plant.mass..g, by=oTreatment) + s(plant.mass..g, by=oTime.point) + s(plant.mass..g, by= otype), data=degra.df, method="REML", select= T)
summary(uv.m1.gam)
...
Plant Decomp Plots (% mass remaining)
```{r}
...
>>>>>>> 057dbc9963d9ace6c887998c4e2184b96dc95b80
...
```{r}
#fit complex model 
uv.m1.gam=gam(mass.remaining.dec ~ Treatment*type*Time.point + s(plant.mass..g, by=oTreatment) + s(plant.mass..g, by=oTime.point) + s(plant.mass..g, by= otype), data=degra.df, method="REML", select= T)
summary(uv.m1.gam)
gam.check(uv.m1.gam, rep = 1000)
draw(uv.m1.gam)

uv.m2.gam=gam(mass.remaining.dec ~ Treatment*type*Time.point + s(plant.mass..g)+ s(plant.mass..g, by=Treatment) + s(plant.mass..g, by=Time.point) + s(plant.mass..g, by= type), data=degra.df, method="REML", select= T)
summary(uv.m2.gam)
gam.check(uv.m2.gam, rep=100)
...
#hereon data is arcsine transformed
uv.m3.gam=gam(mass.remaining.arc ~ Treatment*type*Time.point + s(plant.mass..g) + s(plant.mass..g, by=oTreatment) + s(plant.mass..g, by=oTime.point) + s(plant.mass..g, by= otype), data=degra.df, method="REML", select= T)
summary(uv.m3.gam)
gam.check(uv.m3.gam, rep = 1000)
draw(uv.m3.gam)
...
#better to use ordered factors and no knots
AIC(uv.m1.gam, uv.m2.gam, uv.m3.gam, uv.m4.gam, uv.m5.gam)
...
plot_grid(
  ggdraw(add_sub(suva.plots, "Fig 4. Average specific ultraviolet absorbance (SUVA 254 ) across sampling periods for both treatments. Time-0 represents baseline DOC concentrations \nprior to the start of the experiment. ", x=0.01, hjust=0)), ncol = 1)

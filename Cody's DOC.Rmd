...
```
DOC analysis: model 4 is just looking at treatment effect with time point as random effect
```{r}
library(mgcv)
library(gratia)

m1.lme=lmer(DOC..mg.L ~  1 + Treatment + (1 + Treatment | Time.point), data=DOC.df2)
summary(m1.lme)
plot(m1.lme)


m1.gam=gam(log(DOC..mg.L)~ Treatment + s(plant.mass..g) + s(Time.point, bs = "re"), data = DOC.df2, method = "REML")
gam.check(m1.gam, rep=1000)
summary(m1.gam)
draw(m1.gam)
concrvity(m2.gam)

m2.gam=gam(log(DOC..mg.L)~ Treatment + s(plant.mass..g, by=Treatment) + s(Time.point, bs="re"), data = DOC.df2, select=T, method = "REML")
gam.check(m2.gam, rep=1000) 
summary(m2.gam)
draw(m2.gam)

m3.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, by=Treatment) + s(Time.point, bs="re"), data = DOC.df2, family= Gamma(link = "log"), select= T, method = "REML")
gam.check(m3.gam, rep=1000) 
summary(m3.gam)
draw(m3.gam)
concrvity(m3.gam)

#smooth across plant mass for each level of treatment with time period as random effect—this answers the question: what is treatment effect on DOC while smoothing over plant mass and accounting for variation in random effect of time point?
m4.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, by=Treatment, k=4) + s(Time.point, bs="re"), data = DOC.df2, family= Gamma(link = "log"), select= T, method = "REML")
gam.check(m3.gam, rep=1000) 
summary(m4.gam)
draw(m4.gam)
concrvity(m4.gam)

#time and treatment effects with smooth over plant mass for each level of treatment
m5.gam=gam(DOC..mg.L ~ Treatment+ Time.point + s(plant.mass..g, Treatment, by=Time.point, bs="fs",k=4), data = DOC.df2, family= Gamma(link = "log"), select= T, method = "REML")
...
m6.gam=gam(DOC..mg.L ~ Treatment* Time.point + s(plant.mass..g, Treatment, by=Time.point, bs="fs",k=4), data = DOC.df2, family= Gamma(link = "log"), select= T, method = "REML")
gam.check(m5.gam, rep=1000) 
summary(m6.gam)

AIC(m3.gam, m4.gam, m5.gam, m6.gam)
```

DOC analysis: looking at the effect of time point on DOC without treatment effect
```{r}
m5.gam=gam(DOC..mg.L ~ Time.point + s(plant.mass..g, by=Time.point), data= DOC.df2, method = "REML", select=T, family= Gamma(link = "log"))
summary(m5.gam)
draw(m5.gam)
concrvity(m5.gam)
...
ggplot(DOC.df2, aes(x=exp(predict(m5.gam)), y=DOC..mg.L)) + 
  geom_point(aes(color=exp(predict(m5.gam)))) +
  geom_smooth()+
  labs(x='Predicted Values', y='Actual Values', title='Predicted vs. Actual Values')

library(jtools)
effect_plot(m29.gam, pred = Time.point, interval = TRUE, partial.residuals = TRUE)

gam.check(m29.gam, rep=1000)
...
draw(m28.gam, residuals = TRUE)
summary(m28.gam)

visreg(m5.gam, "plant.mass..g", ylab="DOC..mg.L", trans=exp, gg=TRUE, by="Time.point", overlay=F, partial=F, rug=F, cond = list(Treatment="unburned")) +
  geom_point(alpha=0.5) +
  ggtitle("Burned") +
  ylab("DOC (mg/L)")+
  xlab("plant mass (g)")+

...
vis.burn.plot=visreg(m28.gam, "plant.mass..g", ylab="DOC..mg.L", gg=TRUE, by="Time.point", overlay=T, partial=FALSE, rug=F, cond=list(Treatment="burned")) +
  geom_point(alpha=0.5) +
  ggtitle("Burned") +
  ylab("log(DOC) (mg/L)")+
  xlab("plant mass (g)")+
...
vis.unburn.plot=visreg(m28.gam, "plant.mass..g", ylab="DOC..mg.L", gg=TRUE, by="Time.point", overlay=T, partial=FALSE, rug=F, cond=list(Treatment="unburned")) +
  geom_point(alpha=0.5) +
  ggtitle("Unburned") +
  ylab("log(DOC) (mg/L)")+
  xlab("plant mass (g)")+
...
plot_grid(
  ggdraw(add_sub(vis.plots, "Fig 5. The estimated relationship of dissolved organic carbon (DOC) across sampling period for a) burned and b) unburned treatments. Plots show \na confidence band about the estimates, and the partial residuals to assess the model fit.", x=0.01, hjust=0)), ncol = 1)


ggsave("figures/fig5.DOC.visplot.pdf", height=5, width=14)

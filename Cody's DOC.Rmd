...
plot_grid(
  ggdraw(add_sub(DOC.plots, "Fig 1. Measured Dissolved Organic Carbon (DOC) concentrations across five time points. Time-0 represents baseline DOC concentrations for each treatment prior to the \nstart of the experiment.", x=0.01, hjust=0)), ncol = 1)


ggsave("figures/fig1.DOC.pdf", height=5, width=15)
...
#add absolute value of abs.change.DOC
degra.df$abs.value.DOC=abs(degra.df$abs.change.DOC)
degra.df$abs.percent.DOC=abs(degra.df$percent.change.DOC)
...
plot_grid(
  ggdraw(add_sub(degra.plots, "Fig 2. Three degradation pathways of dissolved organic carbon (DOC) concentrations across three time points during a seven day incubation period for each treatment. \n'Microbe' represents the change in DOC due to microbial degradation, 'photo' represents the change in DOC due to photodegradation, and 'total' represents the change in \nDOC due to both microbial and photodegradation.", x=0.01, hjust=0)), ncol = 1)

degra.plots
ggsave("figures/Fig.2.UVBio.degra_type.plots.pdf", height=5, width=15)
```

Absolute value of abs.change.DOC plots
```{r}

#Unburned treatment degradation
abs.unburned.degra.plot=ggplot(degra.df[(degra.df$Treatment=="unburned"),])+ 
   geom_point(aes(x=plant.mass..g,y=abs.change.DOC,color=type,shape=Time.point),alpha=0.5)+
  geom_smooth(aes(x=plant.mass..g, y=abs.change.DOC, color=type), method="loess")+
  scale_color_manual(values = c("springgreen1", "gold", "midnightblue")) + 
  ggtitle("Degradation: unburned") +
  ylab(expression(paste(Delta, " DOC mg/L"))) +
  xlab("plant material (g)") +
  Fig.formatting
abs.unburned.degra.plot

abs.extract.legend2 <- get_legend(
  # create some space to the left of the legend
  abs.unburned.degra.plot + theme(legend.box.margin = margin(0, 0, 0, 10)))


#Burned degradation
abs.burned.degra.plot=ggplot(degra.df[(degra.df$Treatment=="burned"),])+ 
   geom_point(aes(x=plant.mass..g,y=abs.value.DOC,color=type,shape=Time.point),alpha=0.5)+
  geom_smooth(aes(x=plant.mass..g, y=abs.value.DOC, color=type), method="loess")+
  scale_color_manual(values = c("springgreen1", "gold", "midnightblue")) + 
  ggtitle("Degradation: burned") +
  ylab(expression(paste(Delta, " DOC mg/L"))) +
  xlab("plant material (g)") +
  Fig.formatting
abs.burned.degra.plot

#combine them
abs.degra.plots=plot_grid(abs.unburned.degra.plot + theme(legend.position = "none"),
          abs.burned.degra.plot + theme(legend.position = "none"),
          extract.legend2, 
          rel_widths = c(8,8,3),
          ncol=3)
abs.degra.plots
ggsave("figures/UVBio.abs.degra_type.plots.pdf", height=5, width=9)

range(degra.df[degra.df$Treatment=="unburned",]$abs.change.DOC)
...
#big visual
ggplot(degra.df)+
  geom_point(aes(x=plant.mass..g,y=abs.value.DOC,color=type,shape=Time.point))+
  geom_smooth(aes(x=plant.mass..g, y=abs.value.DOC, color=type), method = "gam", formula = y ~ s(x, bs="tp"))
...
degra.df2=data.frame(degra.df)
degra.df2=degra.df2[-c(5)]


concurvity(m11.gam, full = F)


#fit null (similar format as total DOC)
uv.m1.gam=gam(abs.value.DOC ~ Treatment  + type + s(plant.mass..g, Time.point, bs="fs"), data=degra.df,family= gaussian(link = "log"), method="REML")
summary(uv.m1.gam)

uv.m2.gam=gam(abs.value.DOC~ Treatment  + type + s(plant.mass..g, Treatment, by=type, bs="fs") + s(plant.mass..g, Time.point, by=type, bs="fs"), family= tw, data=degra.df, method="REML")
summary(uv.m2.gam)
gam.check(uv.m2.gam, rep=100)

uv.m3.gam=gam(abs.value.DOC~ Treatment  + type + s(plant.mass..g, type, by=Treatment, bs="fs", k=15) + s(plant.mass..g, Time.point, by=type, bs="fs", k=15), family= tw, data=degra.df, method="REML")
summary(uv.m3.gam)

uv.m4.gam=gam(abs.value.DOC~ Treatment  + type + s(plant.mass..g, Treatment, by=type, bs="fs", k=15) + s(plant.mass..g, Time.point, by=type, bs="fs", k=15), family= tw, data=degra.df, method="REML")
summary(uv.m4.gam)

uv.m5.gam=gam(abs.value.DOC~ Treatment  + type + s(plant.mass..g, type, by=Treatment, bs="fs", k=15) + s(plant.mass..g, Time.point, by=type, bs="fs", k=15), family= tw(link = "log"), data=degra.df, method="REML")
summary(uv.m5.gam)
...
uv.m9.gam=gam(abs.value.DOC~ Treatment*type + s(plant.mass..g, type, by=Treatment, bs="fs") + s(plant.mass..g, Treatment, by=type, bs="fs") + s(plant.mass..g, Time.point, by=type, bs="fs"), family=tw(link = "log"), data=degra.df, method="REML")
summary(uv.m9.gam)

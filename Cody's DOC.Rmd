...
```{r}
#Unburned treatment degradation
mass.unburned.decomp.plot=ggplot(plant.df[(plant.df$Treatment=="unburned"),])+ 
  geom_point(aes(x=plant.specific.mass..g,y=mass.remaining,color=type),alpha=0.5)+
  geom_smooth(aes(x=plant.specific.mass..g, y=mass.remaining, color=type), method="loess", alpha=0.1)+
...
#Burned degradation
mass.burned.decomp.plot=ggplot(plant.df[(plant.df$Treatment=="burned"),])+ 
   geom_point(aes(x=plant.specific.mass..g,y=mass.remaining,color=type),alpha=0.5)+
  geom_smooth(aes(x=plant.specific.mass..g, y=mass.remaining, color=type), method="loess", alpha=0.1)+
  scale_color_manual(values = c("olivedrab", "peru")) + 

...
UVBIO.T1.effect<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=plant.mass..g, y=percent.change.DOC, color=Effect, linetype=Treatment))+
  facet_grid(~Effect)+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("darkgreen", "darkorange2", "darkorchid")) + 
...
UVBIO.T1.DOC.table <- ggplotGrob(UVBIO.T1.DOC) # convert to gtable
UVBIO.T1.effect.table <- ggplotGrob(UVBIO.T1.effect) # convert to gtable

uvbio.widths <- UVBIO.T1.DOC.table$widths[1:3] # extract the first three widths, 
                                  # corresponding to left margin, y lab, and y axis
uvbio.effect.widths <- UVBIO.T1.effect.table$widths[1:3] # same for mpg plot
max.widths <- unit.pmax(uvbio.widths, uvbio.effect.widths) # calculate maximum widths
UVBIO.T1.DOC.table$widths[1:3] <- max.widths # assign max. widths to iris gtable
UVBIO.T1.effect.table$widths[1:3] <- max.widths # assign max widths to mpg gtable



plot_grid(UVBIO.T1.DOC.table, 
          UVBIO.T1.effect.table, 
          nrow=2)


ggsave("figures/T1.degra.combo.plot.pdf", height=8, width=7)
...
UVBIO.T1.DOC.abs<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=plant.mass..g, y=abs.change.DOC, color=Effect, linetype=Treatment))+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("darkgreen", "darkorange2", "darkorchid")) + 
  scale_fill_manual(values = c("darkgreen", "darkorange2", "darkorchid")) +
...
extract.legend.uvbio=get_legend(
  # create some space to the left of the legend
  UVBIO.T1.DOC.abs + theme(legend.box.margin = margin(0, 0, 0, 10), legend.title=element_blank()))
...
UVBIO.T1.effect.abs<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=plant.mass..g, y=abs.change.DOC, color=Effect, linetype=Treatment))+
  facet_grid(~Effect)+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("darkgreen", "darkorange2", "darkorchid")) + 
...
UVBIO.T1.effect.abs=UVBIO.T1.effect.abs + labs(tag = "B")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

require(grid) # for unit.pmax()
UVBIO.T1.DOC.table.abs <- ggplotGrob(UVBIO.T1.DOC.abs) # convert to gtable
UVBIO.T1.effect.table.abs <- ggplotGrob(UVBIO.T1.effect.abs) # convert to gtable

uvbio.widths.abs <- UVBIO.T1.DOC.table.abs$widths[1:3] # extract the first three widths, 
                                  # corresponding to left margin, y lab, and y axis
uvbio.effect.widths.abs <- UVBIO.T1.effect.table.abs$widths[1:3] # same for mpg plot
max.widths.abs <- unit.pmax(uvbio.widths.abs, uvbio.effect.widths.abs) # calculate maximum widths
UVBIO.T1.DOC.table.abs$widths[1:3] <- max.widths.abs # assign max. widths to iris gtable
UVBIO.T1.effect.table.abs$widths[1:3] <- max.widths.abs # assign max widths to mpg gtable



plot_grid(UVBIO.T1.DOC.table.abs, 
          UVBIO.T1.effect.table.abs, 
          nrow=2)


ggsave("figures/T1.abs_change.degra.combo.plot.pdf", height=8, width=7)
...
UVBIO.T1.DOC.suva<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=SUVA, y=percent.change.DOC, color=Effect, linetype=Treatment))+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("darkgreen", "darkorange2", "darkorchid")) + 
  scale_fill_manual(values = c("darkgreen", "darkorange2", "darkorchid")) +
...
UVBIO.T1.effect.suva<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=SUVA, y=percent.change.DOC, color=Effect, linetype=Treatment))+
  facet_grid(~Effect)+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("darkgreen", "darkorange2", "darkorchid")) + 
...
plot_grid(UVBIO.T1.DOC.suva, 
          UVBIO.T1.effect.suva, 
          nrow=2)


ggsave("figures/T1.suva.degra.combo.plot.pdf", height=8, width=7)
...
UVBIO.T1.DOC.spec=UVBIO.T1.DOC.spec +labs(tag = "A")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")
...
UVBIO.T1.effect.spec=UVBIO.T1.effect.spec + labs(tag = "B")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")
...
plot_grid(UVBIO.T1.DOC.spec, 
          UVBIO.T1.effect.spec, 
          nrow=2)


ggsave("figures/T1.spec.degra.combo.plot.pdf", height=8, width=7)
...
uv.m1.gam.spec=gam(abs.change.DOC ~ Effect*Treatment +s(Spectral.slope, by= Effect), data=degra.df, method="REML", select= T, subset= Time.point=="T1")
...
mass.sage.decomp.plot=ggplot(plant.df[(plant.df$type=="sage"),], aes(x=plant.specific.mass..g, y=mass.remaining, color=Treatment, linetype=Treatment))+ 
  geom_point(alpha=0.4)+
   geom_smooth(method=gam, formula= y~s(x), aes(fill=Treatment), alpha=0.3) +
 scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
...
#willow decomp
mass.willow.decomp.plot=ggplot(plant.df[(plant.df$type=="willow"),], aes(x=plant.specific.mass..g, y=mass.remaining, color=Treatment, linetype=Treatment))+ 
   geom_point(alpha=0.4)+
   geom_smooth(method=gam, formula= y~s(x), aes(fill=Treatment), alpha=0.3) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
   scale_fill_manual(values = c("darkred", "darkgreen")) + 
...
suva.T1<-ggplot(suva.subset.df[(suva.subset.df$Time.point=="T1"),], aes(x=Level, y=SUVA, fill=Treatment)) +
  geom_bar(stat = "identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-1") +
  xlab("")+
...
suva.T2<-ggplot(suva.subset.df[(suva.subset.df$Time.point=="T2"),], aes(x=Level, y=SUVA, fill=Treatment)) +
  geom_bar(stat = "identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-2") +
  xlab("Plant biomass category")+
  scale_x_discrete(limits = c("Low", "Medium", "High"))+
  ylab("")+
...
suva.T3<-ggplot(suva.subset.df[(suva.subset.df$Time.point=="T3"),], aes(x=Level, y=SUVA, fill=Treatment)) +
  geom_bar(stat = "identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-3") +
  xlab("")+
...
suva.T4<-ggplot(suva.subset.df[(suva.subset.df$Time.point=="T4"),], aes(x=Level, y=SUVA, fill=Treatment)) +
  geom_bar(stat = "identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-4") +
  xlab("")+
...
library(gridExtra)
library(ggpubr)
x.grob <- textGrob("Plant biomass category", 
                   gp=gpar(fontface="bold", col="black"))
...
index.subset.df=index.df %>% 
  between(plant.mass..g.>= 5 & x <= 75 )%>%
  index.df %>%
   between(plant.mass..g. >= 125 & x <= 175) %>%
  index.df %>%
  between(plant.mass..g. >= 225 & x <= 275)

index.subset.df$Level=as.factor(ifelse(index.subset.df$plant.mass..g=="5", "Low",
         ifelse (index.subset.df$plant.mass..g=="125", "Medium",
           ifelse (index.subset.df$plant.mass..g=="225", "High", "High"))))

index.subset.df=index.subset.df[order(index.subset.df$Level),]
...
freshness plots
```{r}
fresh.T0<-ggplot(index.subset.df[(index.subset.df$Time.point=="T0"),], aes(x=Level, y=Freshness.Index, fill=Treatment)) +
  geom_bar(stat="identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-0") +
  xlab("")+
...
fresh.T1<-ggplot(index.subset.df[(index.subset.df$Time.point=="T1"),], aes(x=Level, y=Freshness.Index, fill=Treatment)) +
  geom_bar(stat="identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-1") +
  xlab("")+
...
fresh.T2<-ggplot(index.subset.df[(index.subset.df$Time.point=="T2"),], aes(x=Level, y=Freshness.Index, fill=Treatment)) +
  geom_bar(stat="identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-2") +
  xlab("Plant biomass category")+
...
fresh.T3<-ggplot(index.subset.df[(index.subset.df$Time.point=="T3"),], aes(x=Level, y=Freshness.Index, fill=Treatment)) +
  geom_bar(stat="identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-3") +
  xlab("")+
...
fresh.T4<-ggplot(index.subset.df[(index.subset.df$Time.point=="T4"),], aes(x=Level, y=Freshness.Index, fill=Treatment)) +
  geom_bar(stat="identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-4") +
  xlab("")+
...
fluorescence plots
```{r}
fluoro.T0<-ggplot(index.subset.df[(index.subset.df$Time.point=="T0"),], aes(x=Level, y=Fluroscence.Index, fill=Treatment)) +
  geom_bar(stat="identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-0") +
  xlab("")+
...
fluoro.T1<-ggplot(index.subset.df[(index.subset.df$Time.point=="T1"),], aes(x=Level, y=Fluroscence.Index, fill=Treatment)) +
  geom_bar(stat="identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-1") +
  xlab("")+
...
fluoro.T2<-ggplot(index.subset.df[(index.subset.df$Time.point=="T2"),], aes(x=Level, y=Fluroscence.Index, fill=Treatment)) +
  geom_bar(stat="identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-2") +
  xlab("Plant biomass category")+
...
fluoro.T3<-ggplot(index.subset.df[(index.subset.df$Time.point=="T3"),], aes(x=Level, y=Fluroscence.Index, fill=Treatment)) +
  geom_bar(stat="identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-3") +
  xlab("")+
...
fluoro.T4<-ggplot(index.subset.df[(index.subset.df$Time.point=="T4"),], aes(x=Level, y=Fluroscence.Index, fill=Treatment)) +
  geom_bar(stat="identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-4") +
  xlab("")+
...
fluoro.plots<-plot_grid(
  ...
  fluoro.T2+ theme(legend.position = "none"),
  fluoro.T3+ theme(legend.position = "none"),
  fluoro.T4+ theme(legend.position = "none"),
  extract.legend5,
  rel_widths = c(50,50,50,50,50, 20), ncol=6)

fluoro.plots=fluoro.plots+ labs(tag = "B")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")
...
humic.T0<-ggplot(index.subset.df[(index.subset.df$Time.point=="T0"),], aes(x=Level, y=Humification.Index, fill=Treatment)) +
  geom_bar(stat="identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-0") +
  xlab("")+
...
humic.T1<-ggplot(index.subset.df[(index.subset.df$Time.point=="T1"),], aes(x=Level, y=Humification.Index, fill=Treatment)) +
  geom_bar(stat="identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-1") +
  xlab("")+
...
humic.T2<-ggplot(index.subset.df[(index.subset.df$Time.point=="T2"),], aes(x=Level, y=Humification.Index, fill=Treatment)) +
  geom_bar(stat="identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-2") +
  xlab("Plant biomass category")+
...
humic.T3<-ggplot(index.subset.df[(index.subset.df$Time.point=="T3"),], aes(x=Level, y=Humification.Index, fill=Treatment)) +
  geom_bar(stat="identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-3") +
  xlab("")+
...
humic.T4<-ggplot(index.subset.df[(index.subset.df$Time.point=="T4"),], aes(x=Level, y=Humification.Index, fill=Treatment)) +
  geom_bar(stat="identity", position = "dodge")+
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Time-4") +
  xlab("")+
...
index.combo.plots=plot_grid(fresh.plots, fluoro.plots, humic.plots, suva.plots, nrow=4)

...
Visualize it
```{r}
ggplot(degra.df[(degra.df$Treatment=="unburned"),])+
  geom_point(aes(x=plant.mass..g,y=abs.change.DOC,color=type,shape=Time.point))+
  geom_smooth(aes(x=plant.mass..g, y=abs.change.DOC, color=type), method = "gam", formula = y ~ s(x, bs="tp"))
...
degra.plots=plot_grid(unburned.degra.plot + theme(legend.position = "none"),
          ...
          ncol=3)
degra.plots
ggsave("figures/UVBio.degra_type.plots.pdf", height=5, width=9)
```

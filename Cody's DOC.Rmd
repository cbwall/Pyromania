...
plot_grid(
  ggdraw(add_sub(DOC.plots, "Fig 1. Measured dissolved organic carbon (DOC) concentrations across five time points for both treatments. Time-0 represents baseline DOC concentrations prior to the \nstart of the experiment.", x=0.01, hjust=0)), ncol = 1)
...
total.df=data.frame(UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$Time.point,
                    ...
                    UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$DOC..mg.L-UVBIO.df[UVBIO.df$Filt.UV=="Filtered.NoUV",]$DOC..mg.L,
                     (100*(UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$DOC..mg.L-UVBIO.df[UVBIO.df$Filt.UV=="Filtered.NoUV",]$DOC..mg.L)/(UVBIO.df[UVBIO.df$Filt.UV=="Filtered.NoUV",]$DOC..mg.L)))
total.df$type="total"
...
#combine them
perc.degra.plots=plot_grid(perc.unburned.degra.plot + theme(legend.position = "none"), perc.burned.degra.plot + theme(legend.position = "none"),
          extract.legend2,
          labels = c("a)", "b)"),
          ncol=3, rel_widths = c(12,12,3))

plot_grid(
  ggdraw(add_sub(perc.degra.plots, "Fig 2. Three degradation pathways of dissolved organic carbon (DOC) across three time points during a seven day incubation period for both treatments. 'Microbe' \nrepresents the percent change in DOC due to microbial degradation, 'photo' represents the percent change in DOC due to photodegradation, and 'total' represents \nthe percent change in DOC due to both microbial and photodegradation.", x=0.01, hjust=0)), ncol = 1)

degra.plots
ggsave("figures/Fig.2.UVBio.perc.degra_type.plots.pdf", height=5, width=15)

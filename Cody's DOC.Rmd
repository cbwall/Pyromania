...
ggsave("figures/DOC.concentration.pdf", height=5, width=14)
...
library(mgcv)
library(gratia)
m1.gam=gam(DOC..mg.L~ Treatment + s(plant.mass..g, by=Treatment), subset = Time.point=="T1", family= Gamma(link = "log"), data = DOC.df2, method = "REML")
summary(m1.gam)
...
###########
m1b.gam=gam(DOC..mg.L~ Treatment + s(plant.mass..g, by=Treatment), subset = Time.point=="T1", family= Gamma(link = "log"), data = DOC.df2, method = "REML", select=T)
summary(m1b.gam)
draw(m1b.gam)
gam.check(m1b.gam, rep=1000)
anova(m1b.gam)

###########
m1c.gam=gam(DOC..mg.L~ Treatment + s(plant.mass..g, by=Treatment, bs="cs"), subset = Time.point=="T1", family= Gamma(link = "log"), data = DOC.df2, method = "REML", select=T)
summary(m1c.gam)


m1d.gam=gam(DOC..mg.L~ Treatment + s(plant.mass..g) + s(plant.mass..g, by=Treatment,m=1), subset = Time.point=="T1", family= Gamma(link = "log"), data = DOC.df2, method = "REML", select=T)
summary(m1d.gam)
draw(m1d.gam)

AIC(m1b.gam,m1c.gam,m1d.gam )
```

T2 model
```{r}
m2.gam=gam(DOC..mg.L~ Treatment + s(plant.mass..g, by=Treatment, bs="cr"), subset = Time.point=="T2", family= Gamma(link = "log"), data = DOC.df2, method = "REML", select = T)
summary(m2.gam)
gam.check(m2.gam, rep=1000)

#######
m2b.gam=gam(DOC..mg.L~ Treatment + s(plant.mass..g, by=Treatment, bs="cr"), subset = Time.point=="T2", family= Gamma(link = "log"), data = DOC.df2, method = "REML")
######
...
```{r}
#####
m3.gam=gam(DOC..mg.L~ Treatment + s(plant.mass..g, by=Treatment, bs="cr", k=6), subset = Time.point=="T3", family= Gamma(link = "log"), data = DOC.df2, method = "REML", select = T)
summary(m3.gam)
draw(m3.gam)
######

m3b.gam=gam(DOC..mg.L~ Treatment + s(plant.mass..g, by=Treatment, bs="cs"), subset = Time.point=="T3", family= Gamma(link = "log"), data = DOC.df2, method = "REML", select=T)
summary(m3.gam)

m3c.gam=gam(DOC..mg.L~ Treatment + s(plant.mass..g) + s(plant.mass..g, by=Treatment, bs="ps", m=1), subset = Time.point=="T3", family= Gamma(link = "log"), data = DOC.df2, method = "REML")

AIC(m3.gam, m3b.gam, m3c.gam)
```

T4 model
```{r}

m4.gam=gam(DOC..mg.L~ Treatment + s(plant.mass..g, by=Treatment, bs="ps", k=4), subset = Time.point=="T4", family= Gamma(link = "log"), data = DOC.df2, method = "REML")
summary(m4.gam)
gam.check(m4.gam, rep=1000)


####
m4b.gam=gam(DOC..mg.L~ Treatment + s(plant.mass..g, by=Treatment, k=6, bs="ps"), subset = Time.point=="T4", family= Gamma(link = "log"), data = DOC.df2, method = "REML", select=T)
summary(m4b.gam)
#####
AIC(m4.gam, m4b.gam)
```
...
```{r}
summary(m5.gam)$coefficient

#Let’s go through each coefficient:

#(Intercept): exp(2.28823)= 9.86 mean DOC for T1 burned
#Treatmentunburned: difference between mean DOC T1 unburned exp(0.13288) vs T1 burned = 9.86-1.14= 8.72
#T2: the difference in DOC between T2 exp(-0.17301) versus T1 for burned = 9.86-0.84= 9.02
...
library(visreg)
visreg(m1b.gam, "plant.mass..g", ylab="DOC..mg.L", trans=exp, gg=TRUE, by="Treatment", overlay=F, partial=F, rug=F, cond = list(Treatment="burned")) +
  geom_point(alpha=0.5) +
  ggtitle("Burned") +
  ylab("DOC (mg/L)")+
  xlab("plant mass (g)")+
...
MODEL 28 visualization of fitted
```{r}
#we can see what this looks like
library(visreg)


vis.burn.plot=visreg(m32.gam, "plant.mass..g", ylab="DOC..mg.L", trans=exp, gg=TRUE, by="Time.point", overlay=T, partial=T, rug=T, cond=list(Treatment="burned")) +
  geom_point(alpha=0.5) +
  ggtitle("Burned") +
  ylab("DOC (mg/L)")+
  xlab("plant mass (g)")+
...
vis.unburn.plot=visreg(m28.gam, "plant.mass..g", ylab="DOC..mg.L", gg=TRUE, trans=exp, by="Time.point", overlay=T, partial=F, rug=F, cond=list(Treatment="unburned")) +
  geom_point(alpha=0.5) +
  ggtitle("Unburned") +
  ylab("DOC (mg/L)")+
  xlab("plant mass (g)")+
...
extract.legend.vis <- get_legend(
  # create some space to the left of the legend
  vis.burn.plot + theme(legend.box.margin = margin(0, 0, 0, 10)))


vis.plots<-plot_grid(
  vis.burn.plot+ theme(legend.position = "none"),
  vis.unburn.plot+ theme(legend.position = "none"),
  extract.legend.vis,
  labels = c("a)", "b)"),
  rel_widths = c(18,18,8), ncol=3)
vis.plots

plot_grid(
  ggdraw(add_sub(vis.plots, "Fig 5. The estimated relationship of dissolved organic carbon (DOC) across sampling period for a) burned and b) unburned \ntreatments. Plots show a confidence band about the exponentiated estimates, and the partial residuals to assess the model fit.", x=0.01, hjust=0)), ncol = 1)


ggsave("figures/fig5.DOC.visplot.pdf", height=5, width=11)
...
mass.unburned.degra.plot=ggplot(degra.df[(degra.df$Treatment=="unburned"),])+ 
   geom_point(aes(x=plant.mass..g,y=mass.remaining,color=type),alpha=0.5)+
  geom_smooth(aes(x=plant.mass..g, y=mass.remaining, color=type), method="loess", alpha=0.1)+
  facet_grid(~Time.point)+
  scale_color_manual(values = c("springgreen1", "gold", "midnightblue")) + 
  ggtitle("Degradation: unburned") +
...
uv.m1.gam=gam(mass.remaining.arc ~ effect*Treatment + s(plant.mass..g, by=effect, k=6, bs="cr"), data=degra.df, method="REML", select= T, subset= Time.point=="T1")
summary(uv.m1.gam)
concrvity(uv.m1.gam)



uv.m1b.gam=gam(mass.remaining.arc ~ effect*Treatment +  s(plant.mass..g, by=effect, k=4, bs="cr") + s(plant.mass..g, by= Treatment, bs="cr"), data=degra.df, method="REML", select= T, subset= Time.point=="T1")
summary(uv.m1b.gam)
draw(uv.m1b.gam)
gam.check(uv.m1b.gam, rep= 1000)
mgcv::concurvity(uv.m1b.gam, full=F)
plot(uv.m1b.gam, pages = 1)

#########
uv.m1c.gam=gam(mass.remaining.arc ~ effect*Treatment + s(plant.mass..g, bs="cr", k=4) + s(plant.mass..g, by=effect, k=4, bs="cr", m=1) + s(plant.mass..g, by= Treatment, bs="cr", m=1), data=degra.df, method="REML", select= T, subset= Time.point=="T1")
mgcv::concurvity(uv.m1c.gam, full=F)
summary(uv.m1c.gam)
draw(uv.m1c.gam)
gam.check(uv.m1c.gam, rep=1000)
########3

uv.m1d.gam=gam(abs.change.DOC^2 ~ effect*Treatment + s(plant.mass..g, bs="cr", k=4) + s(plant.mass..g, by=effect, k=4, bs="cr", m=1) + s(plant.mass..g, by= Treatment, bs="cr", m=1), data=degra.df, method="REML", select= T, subset= Time.point=="T1")
gam.check(uv.m1d.gam, rep=1000)
summary(uv.m1d.gam)
draw(uv.m1d.gam)


uv.m1e.gam=gam(mass.remaining.arc ~ effect*Treatment + s(plant.mass..g) + s(plant.mass..g, by=Treatment, m=1, k=6) + s(plant.mass..g, by=effect, m=1, k=6), data=degra.df, method="REML", select= T, subset= Time.point=="T1")
gam.check(uv.m1e)
summary(uv.m1e.gam)
draw(uv.m1e.gam)
plot(uv.m1e.gam, residuals = T)
concurvity(uv.m1e.gam)


uv.m1f.gam=gam(mass.remaining.arc ~ effect*Treatment + s(plant.mass..g) + s(plant.mass..g, by=Treatment, m=1, k=12) + s(plant.mass..g, by=effect, m=1, k=12), data=degra.df, method="REML", select= T, subset= Time.point=="T1")
summary(uv.m1d.gam)
gam.check(uv.m1f.gam, rep=1000)
plot(uv.m1d.gam)

uv.m1g.gam=gam(mass.remaining.arc ~ effect*Treatment  + s(plant.mass..g, by=Treatment, k=12) + s(plant.mass..g, by=effect, k=12), data=degra.df, method="REML", select= T, subset= Time.point=="T1")


AIC( uv.m1.gam, uv.m1b.gam, uv.m1c.gam)
...
visualize uv.model1c
```{r}
uv.t1.burned=visreg(uv.m1c.gam, "plant.mass..g", gg=TRUE, by="effect", overlay=T, partial=F, rug=F, cond=list(Treatment="burned")) +
  ggtitle("T1: Burned") +
  xlab("plant mass (g)")+
  theme_cowplot(12)

uv.t1.unburned=visreg(uv.m1c.gam, "plant.mass..g", gg=TRUE, by="effect", overlay=T, partial=F, rug=F, cond=list(Treatment="unburned")) +
  ggtitle("T1: Unburned") +
  xlab("plant mass (g)")+
  theme_cowplot(12)
...
hist(log10(DOC.df2[DOC.df2$Time.point=="T4",]$DOC..mg.L))


friedman.test(mass.remaining.arc~type|plant.mass..g, data=degra.df, subset = degra.df$Time.point=="T1")
...
T2 model
```{r}
uv.m2.gam=gam(mass.remaining.arc ~ effect*Treatment + s(plant.mass..g, by=effect, k=4, bs="ts"), data=degra.df, method="REML", select= T, subset= Time.point=="T2")
summary(uv.m2.gam)
mgcv::concurvity(uv.m2b.gam, full=F)
draw(uv.m2.gam)
...
```{r}
uv.t2.burned=visreg(uv.m2d.gam, "plant.mass..g", gg=TRUE, by="effect", overlay=T, partial=F, rug=F, cond=list(Treatment="burned")) +
  ggtitle("T2: Burned") +
  xlab("plant mass (g)")+
  theme_cowplot(12)

uv.t2.unburned=visreg(uv.m2d.gam, "plant.mass..g", gg=TRUE, by="effect", overlay=T, partial=F, rug=F, cond=list(Treatment="unburned")) +
  ggtitle("T2: Unburned") +
  xlab("plant mass (g)")+
  theme_cowplot(12)
...
T3 model
```{r}
uv.m3.gam=gam(mass.remaining.arc ~ effect*Treatment + s(plant.mass..g, by=effect, k=4, bs="ts") + s(plant.mass..g, by= Treatment, bs="ts"), data=degra.df, method="REML", select= T, subset= Time.point=="T3")
summary(uv.m3.gam)
mgcv::concurvity(uv.m2c.gam, full=F)
...
summary(uv.m3c.gam)
mgcv::concurvity(uv.m3c.gam, full=F)
...
```{r}
uv.t3.burned=visreg(uv.m3b.gam, "plant.mass..g", gg=TRUE, by="effect", overlay=T, partial=F, rug=F, cond=list(Treatment="burned")) +
  ggtitle("T3: Burned") +
  xlab("plant mass (g)")+
  theme_cowplot(12)

uv.t3.unburned=visreg(uv.m3b.gam, "plant.mass..g", gg=TRUE, by="effect", overlay=T, partial=F, rug=F, cond=list(Treatment="unburned")) +
  ggtitle("T3: Unburned") +
  xlab("plant mass (g)")+
  theme_cowplot(12)
...
sage.df=data.frame(decomp.df[decomp.df$plant.type=="sage",]$treatment,
                    ...
                    decomp.df[decomp.df$plant.type=="sage",]$dry.mass..g..w.o.litter.bag.or.foil.-decomp.df[decomp.df$plant.type=="sage",]$plant.specific.mass..g,
                     (100*(decomp.df[decomp.df$plant.type=="sage",]$dry.mass..g..w.o.litter.bag.or.foil.-decomp.df[decomp.df$plant.type=="sage",]$plant.specific.mass..g)/(decomp.df[decomp.df$plant.type=="sage",]$plant.specific.mass..g)))
sage.df$type="sage"
...
willow.df=data.frame(decomp.df[decomp.df$plant.type=="willow",]$treatment,
                    ...
                    decomp.df[decomp.df$plant.type=="willow",]$dry.mass..g..w.o.litter.bag.or.foil.-decomp.df[decomp.df$plant.type=="willow",]$plant.specific.mass..g,
                     (100*(decomp.df[decomp.df$plant.type=="willow",]$dry.mass..g..w.o.litter.bag.or.foil.-decomp.df[decomp.df$plant.type=="willow",]$plant.specific.mass..g)/(decomp.df[decomp.df$plant.type=="willow",]$plant.specific.mass..g)))
...
mass.unburned.decomp.plot=ggplot(plant.df[(plant.df$type=="sage"),])+ 
  ...
  geom_smooth(aes(x=plant.specific.mass..g, y=mass.remaining, color=Treatment), method="loess", alpha=0.1)+
  scale_color_manual(values = c("orange", "purple")) + 
...
degra.plots
ggsave("figures/mass.decomp_type.plots.pdf", height=5, width=10)

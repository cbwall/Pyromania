...
```{r}
m2.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, by=Treatment, k=8, m=1) + s(plant.mass..g, Time.point, k=11, bs="fs", m=1), data=DOC.df, method="REML", select = T)
summary(m2.gam)
draw(m2.gam)
k.check(m2.gam)

library(visreg)
visreg(m2.gam, xvar = "plant.mass..g",
       by = "Treatment", data = DOC.df,
       method = "REML")
```

```{r}
m3.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, bs="tp", k=8) + s(plant.mass..g, by=Treatment, k=8, m=1) + s(plant.mass..g, Time.point, k=8, bs="fs", m=1), data=DOC.df, method="REML", select = T)
summary(m3.gam)
draw(m3.gam)
...
```{r}
#with link= log
m4.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, by=Treatment, k=8, m=1) + s(plant.mass..g, Time.point, k=8, bs="fs", m=1),family = gaussian(link = "log"), data=DOC.df, method="REML", select=T)
summary(m4.gam)
draw(m4.gam)
```

```{r}
m5.gam=gam(DOC..mg.L~ Treatment + s(plant.mass..g, bs="tp", k=8, m=1)+ s(plant.mass..g, Time.point, bs="fs", k=8, m=1),#time point specific trends
           data= DOC.df,
           methods= 'REML',
           select=TRUE)
summary(m5.gam)

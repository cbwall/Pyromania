---
title: "Cody"
author: "Cody Spiegel"
date: "4/22/2022"
output: word_document
---

```{r setup chunk, setup, include = FALSE, cache=FALSE, message=FALSE, warning=FALSE}
if (!require('knitr')) install.packages('knitr'); library('knitr')
knitr::opts_chunk$set(warning=FALSE, message=FALSE, fig.align='center')

# load packages
if (!require("pacman")) install.packages("pacman") # for rapid install if not in library

# use pacman to load all the packages you are missing!
pacman::p_load('knitr', 'lme4', 'lmerTest', 'tidyverse', 'magrittr', 'effects', 'plyr', 'dplyr', 
               'plotrix', 'car',"gridExtra", "cowplot", "tools")
Fig.formatting<-(theme_classic()) +
  theme(text=element_text(size=14),
        legend.text.align = 0,
        panel.border = element_rect(fill=NA, colour = "black", size=1),
        aspect.ratio=1, 
        axis.ticks.length=unit(0.25, "cm"),
        axis.text.y=element_text(
        margin=unit(c(0.5, 0.5, 0.5, 0.5), "cm"), colour="black", size=12), 
        axis.text.x=element_text(
        margin=unit(c(0.5, 0.5, 0.5, 0.5), "cm"), colour="black", size=12)) +
  theme(legend.key.size = unit(0.4, "cm")) +
  theme(aspect.ratio=1.3) +
  theme(panel.spacing=unit(c(0, 0, 0, 0), "cm"))


```

###DOC  
import and do a loop to clean up all files and make stacked data in single df
```{r}
## import treatment IDs
IDs<-read.csv("/Users/Cody1/Desktop/GitHub/Pyromania/data/treatment.IDs.csv")

##### grab files in a list
Total.DOC.files <- list.files(path="/Users/Cody1/Desktop/GitHub/Pyromania/data/DOC.TN", pattern = "csv$", full.names = T)
Total.DOC.files
##### what are the file names, sans extensions using package 'tools'
file.names<-file_path_sans_ext(list.files(path="/Users/Cody1/Desktop/GitHub/Pyromania/data/DOC.TN", pattern = "csv$", full.names = F))
file.names

############ formatting all data in for loop
  for(i in 1:length(Total.DOC.files))
    {
  data<-read.csv(Total.DOC.files[i], sep=",")
  data<-data[,c(1,3,4)] # removed columns we don't need
  data$File<-Total.DOC.files[i]
  colnames(data)<-c("Tank", "DOC..mg.L", "TN..mg.L", "File")
  data$Tank<- IDs$Tank
  data$Tank<-as.numeric(as.character(data$Tank)) # make the column of samples all numeric
  data <- data[!is.na(as.numeric(as.character(data$Tank))),] # remove all rows that aren't numeric/tanks
  data$Treatment<-IDs$Treatment
  data$plant.mass..g<-IDs$plant.mass..g
  make.names(assign(paste(file.names[i], sep=""), data)) # make the file name the name of new df for loop df
  }

########## this is the end of the loop

#Combine files from loop to single df

DOC.df<-rbind(DOC_T0, DOC_T1, DOC_T2, DOC_T3, DOC_T4)

#Extract file
DOC.df$File=sapply(strsplit(DOC.df$File, "/"), `[`, 9)

# alternative way to code the above
#give the 10th-24th character of the file name, removing the rest
#DOC.df$File<-substr(DOC.df$File, 13, 27) 

#alternatively
# remove the 9 letters ('^.) at start 
# remove the 4 letters (.$') at end
#DOC.df$File<-gsub('^.........|....$', '', DOC.df$File) 

# if equals DOC_T0 then, T0, if not then T1..etc
DOC.df$Time.point<- as.factor(ifelse(DOC.df$File=="DOC_T0.csv", "T0",
         ifelse (DOC.df$File=="DOC_T1.csv", "T1",
           ifelse (DOC.df$File=="DOC_T2.csv", "T2", 
                   ifelse(DOC.df$File=="DOC_T3.csv", "T3", "T4")))))


#rearrange
library(dplyr)
DOC.df<- DOC.df %>% 
  dplyr::select(File, Time.point, Treatment, Tank, plant.mass..g, DOC..mg.L, TN..mg.L) 


DOC.df$Treatment=as.factor(DOC.df$Treatment)
DOC.df$Time.point=as.factor(DOC.df$Time.point)
DOC.df$plant.mass..g=as.numeric(DOC.df$plant.mass..g)




mean(DOC_T1[DOC_T1$Treatment=="burned",]$DOC..mg.L)
mean(DOC_T4[DOC_T4$Treatment=="burned",]$DOC..mg.L)

mean(DOC_T1[DOC_T1$Treatment=="unburned",]$DOC..mg.L)
mean(DOC_T4[DOC_T4$Treatment=="unburned",]$DOC..mg.L)


```


DOC analysis: 
T1 model
```{r}
library(mgcv)
library(gratia)
library(car)

#Time 0
m0.DOC.treatment=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, by= Treatment), subset = Time.point=="T0", data = DOC.df, method = "REML")

summary(m0.DOC.treatment)

m0.DOC.global=gam(DOC..mg.L ~ Treatment + s(plant.mass..g), subset = Time.point=="T0", data = DOC.df, method = "REML")

m0.DOC.global.max=gam(DOC..mg.L ~ s(plant.mass..g), subset = Time.point=="T0", data = DOC.df, method = "REML")

AIC(m0.DOC.treatment, m0.DOC.global, m0.DOC.global.max)


#Time 1
m1.DOC.treatment=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, by= Treatment), subset = Time.point=="T1", data = DOC.df, method = "REML")

m1.DOC.global=gam(DOC..mg.L ~ Treatment + s(plant.mass..g), subset = Time.point=="T1", data = DOC.df, method = "REML")

m1.DOC.global.max=gam(DOC..mg.L ~ s(plant.mass..g), subset = Time.point=="T1", data = DOC.df, method = "REML")

AIC(m1.DOC.treatment, m1.DOC.global, m1.DOC.global.max)


#Time 2
m2.gam.treatment=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, by= Treatment), subset = Time.point=="T2", data = DOC.df, method = "REML")

m2.gam.global=gam(DOC..mg.L ~ Treatment + s(plant.mass..g), subset = Time.point=="T2", data = DOC.df, method = "REML")

m2.gam.global.max=gam(DOC..mg.L ~ s(plant.mass..g), subset = Time.point=="T2", data = DOC.df, method = "REML")

AIC(m2.gam.treatment, m2.gam.global, m2.gam.global.max)

#Time3
m3.gam.treatment=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, by= Treatment), subset = Time.point=="T3", data = DOC.df, method = "REML")

m3.gam.global=gam(DOC..mg.L ~ Treatment + s(plant.mass..g), subset = Time.point=="T3", data = DOC.df, method = "REML")

m2.gam.global.max=gam(DOC..mg.L ~ s(plant.mass..g), subset = Time.point=="T3", data = DOC.df, method = "REML")


#Time 4
m4.gam.treatment=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, by= Treatment), subset = Time.point=="T4", data = DOC.df, method = "REML")

m4.gam.global=gam(DOC..mg.L ~ Treatment + s(plant.mass..g), subset = Time.point=="T4", data = DOC.df, method = "REML")

m4.gam.global.max=gam(DOC..mg.L ~ s(plant.mass..g), subset = Time.point=="T4", data = DOC.df, method = "REML")

AIC(m4.gam.treatment, m4.gam.global, m4.gam.global.max)

 anova(m1.gam)
summary(m1.gam)
anova.gam(m1.gam)
gam.check(m1.gam, rep=1000)
draw(m1.gam)
concrvity(m1.gam)
par(mfrow = c(1, 2))
plot(m1.gam, all.terms = TRUE, page=1)

#CI
ci <- confint(m1.gam, type = "confidence")
head(ci)
```


DOC plots: biomass is a proxy for DOC 
```{r}
library(ggtext)
library(mgcv)
library(gratia)
library(tidymv)

DOC.T0<-plot_smooths(
  model = m0.DOC.treatment,
  series = plant.mass..g,
  comparison = Treatment
) + 
  geom_point(data=DOC.df[(DOC.df$Time.point=="T0"),], 
             aes(x=plant.mass..g, y=DOC..mg.L, color=Treatment)) +
  scale_color_manual(values = c("brown1", "mediumseagreen")) + 
  scale_fill_manual(values = c("brown1", "mediumseagreen")) + 
  coord_cartesian(ylim=c(0, 60)) +
  ggtitle("Day 10") +
  ylab(expression("DOC mg" ~L^-1))+
  xlab("plant material (g)") +
  Fig.formatting

DOC.T0
  

plot_smooths(
  model = m1.DOC.treatment,
  series = plant.mass..g,
  comparison = Treatment
) + 
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  coord_cartesian(ylim=c(0, 60)) +
  ggtitle("Time-0") +
  xlab("")+
  ylab(expression("DOC mg" ~L^-1))+
  Fig.formatting
  


DOC.T1<-ggplot(DOC.df[(DOC.df$Time.point=="T1"),], aes(x=plant.mass..g, y=DOC..mg.L, color=Treatment, linetype=Treatment)) + 
  geom_point(alpha=0.4) +
   scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-1") +
xlab("Plant mass (g)")+
    ylab(expression("DOC mg" ~L^-1))+
  coord_cartesian(ylim=c(0, 60)) +
  Fig.formatting

DOC.T1




DOC.T2<-ggplot(DOC.df[(DOC.df$Time.point=="T2"),], aes(x=plant.mass..g, y=DOC..mg.L, color=Treatment, linetype=Treatment)) + 
  geom_point(alpha=0.4) +
   scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-2") +
  xlab("Plant mass (g)") +
  ylab("")+
  coord_cartesian(ylim=c(0, 60)) +
    Fig.formatting


  
DOC.T3<-ggplot(DOC.df[(DOC.df$Time.point=="T3"),], aes(x=plant.mass..g, y=DOC..mg.L, color=Treatment, linetype=Treatment)) + 
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  coord_cartesian(ylim=c(0, 60)) +
  ggtitle("Time-3") +
  xlab("")+
  ylab("")+
    Fig.formatting



DOC.T4<-ggplot(DOC.df[(DOC.df$Time.point=="T4"),], aes(x=plant.mass..g, y=DOC..mg.L, color=Treatment, linetype=Treatment)) + 
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  coord_cartesian(ylim=c(0, 60)) +
  ggtitle("Time-4") +
  xlab("")+
  ylab("")+
    Fig.formatting



extract.legend <- get_legend(
  # create some space to the left of the legend
  DOC.T0 + theme(legend.box.margin = margin(0, 0, 0, 10)))


DOC.plots<-plot_grid(
  DOC.T0+ theme(legend.position = "none"),
  DOC.T1+ theme(legend.position = "none"),
  DOC.T2+ theme(legend.position = "none"),
  DOC.T3+ theme(legend.position = "none"),
  DOC.T4+ theme(legend.position = "none"),
  extract.legend,
  rel_widths = c(50,50,50,50, 50, 20), ncol=6)

DOC.plots=DOC.plots+labs(tag = "A")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

DOC.plots

ggsave("figures/DOC.concentration.pdf", height=5, width=17.5)


plot_grid(
  ggdraw(add_sub(DOC.plots, "Fig 3. Measured dissolved organic carbon (DOC) concentrations across five sampling periods. Color and line type included to distinguish individual plant trends. Time-0 represents baseline DOC concentrations before the addition of plant material with Time-1 (p< 0.49), Time-2 (p= 0.83), Time-3 (p< 0.01), and Time-4 were sampled for both treatments in November 2021, December 2021, January 2022, and February 2022, respectively. ", x=0.01, hjust=0)), ncol = 1)




```

Total N plots
```{r}
TN.T0<-ggplot(DOC.df[(DOC.df$Time.point=="T0"),], aes(x=plant.mass..g, y=TN..mg.L, color=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-0") +
  xlab("")+
  ylab("TDN concentration (mg/L)")+
  Fig.formatting+
  coord_cartesian(ylim = c(0,1.5))

 
TN.T1<-ggplot(DOC.df[(DOC.df$Time.point=="T1"),], aes(x=plant.mass..g, y=TN..mg.L, color=Treatment)) +
  geom_point(alpha=0.4) +
   scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-1") +
  xlab("")+
  ylab("")+
  Fig.formatting+
  coord_cartesian(ylim = c(0,1.5))
  
 
TN.T2<-ggplot(DOC.df[(DOC.df$Time.point=="T2"),], aes(x=plant.mass..g, y=TN..mg.L, color=Treatment)) +
  geom_point(alpha=0.4) +
   scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-2") +
  xlab("Plant mass (g)")+
  ylab("")+
  Fig.formatting+
  coord_cartesian(ylim = c(0,1.5))

 

TN.T3<-ggplot(DOC.df[(DOC.df$Time.point=="T3"),], aes(x=plant.mass..g, y=TN..mg.L, color=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-3") +
  xlab("")+
  ylab("")+
  Fig.formatting+
  coord_cartesian(ylim = c(0,1.5))
 

TN.T4<-ggplot(DOC.df[(DOC.df$Time.point=="T4"),], aes(x=plant.mass..g, y=TN..mg.L, color=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-4") +
  xlab("")+
  ylab("")+
  Fig.formatting+
  coord_cartesian(ylim = c(0,1.5))
  
TN.plots<-plot_grid(
  TN.T0+ theme(legend.position = "none"),
  TN.T1+ theme(legend.position = "none"),
  TN.T2+ theme(legend.position = "none"),
  TN.T3+ theme(legend.position = "none"),
  TN.T4+ theme(legend.position = "none"),
  extract.legend, 
  rel_widths = c(50,50,50,50,50,20), ncol=6)

TN.plots=TN.plots+labs(tag = "B")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")



ggsave("figures/TN.concentration.pdf", height=5, width=17.5)

```

Combined DOC/TN plots

```{r}

DOC.TN.plots= plot_grid(DOC.plots, TN.plots, nrow=2)

DOC.TN.plots

ggsave("figures/DOC.TN.plots.pdf", height=7, width=14.5)



```


DOC varies non-linearly across the plant mass gradient, with highest levels of DOC in intermediate/high mass tanks.

There is also a pronounced difference in DOC between the treatment, with high levels in unburned, compared to burned.

T2 model
```{r}
m2.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, by=Treatment), subset = Time.point=="T2", data = DOC.df2, method = "REML")

m2.gam.wo=gam(DOC..mg.L ~ s(plant.mass..g, by= Treatment), subset = Time.point=="T2", data = DOC.df2, method = "REML")



AIC(m2.gam, m2.gam.wo)

summary(m2.gam)
anova.gam(m2.gam)
gam.check(m2.gam, rep=1000)
draw(m2.gam)
concrvity(m2.gam)
par(mfrow = c(1, 2))
plot(m2.gam, all.terms = TRUE, page=1)
```
DOC varies non-linearly across the plant mass gradient, with highest levels of DOC in intermediate/high mass tanks still.

There is also no difference in DOC between the treatment.


T3 model
```{r}
m3.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, by=Treatment), subset = Time.point=="T3", data = DOC.df2, method = "REML")
summary(m3.gam)
anova.gam(m3.gam)
gam.check(m3.gam, rep=1000)
draw(m3.gam)
concrvity(m3.gam)
par(mfrow = c(1, 2))
plot(m3.gam, all.terms = TRUE, page=1)
```
DOC varies non-linearly across the plant mass gradient, with highest levels of DOC in intermediate/high mass tanks still.

There is also significant difference in DOC between the treatment, with highest level in the burned treatment compared to the unburned.


T4 model
```{r}
m4.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, by=Treatment), subset = Time.point=="T4", data = DOC.df2, method = "REML")
summary(m4.gam)
anova.gam(m4.gam)
gam.check(m4.gam, rep=1000)
draw(m4.gam)
concrvity(m4.gam)
par(mfrow = c(1, 2))
plot(m4.gam, all.terms = TRUE, page=1)
```
DOC varies non-linearly across the plant mass gradient, with highest levels of DOC in intermediate/high mass tanks still.

There is also significant difference in DOC between the treatment, with highest level in the burned treatment compared to the unburned.


```{r}
m.treatment.gam=gam(DOC..mg.L~Treatment + s(plant.mass..g), subset = Time.point=="T1", data=DOC.df2)
summary(m.treatment.gam)
anova(m.treatment.gam)

m.treatment.gam2=gam(DOC..mg.L~Treatment + s(plant.mass..g), subset = Time.point=="T2", data=DOC.df2)
summary(m.treatment.gam)
anova(m.treatment.gam2)

#box plots of DOC
DOC.boxplot=ggplot(DOC.df, aes(x=Treatment , y=DOC..mg.L, fill=Treatment)) +
  geom_boxplot(aes(fill=Treatment))+
  facet_grid(~Time.point)+
  scale_fill_manual(values = c("orange", "purple")) + 
  xlab("Treatment")+
  ylab("DOC concentration (mg/L)")+
  theme_cowplot(12)+
  theme(legend.title=element_blank())
  


```


T2 model
```{r}
m1.TN.gam=gam(TN..mg.L ~ Treatment + s(plant.mass..g, by=Treatment), subset = Time.point=="T1", data = DOC.df2, method = "REML")
summary(m1.TN.gam)
anova.gam(m1.TN.gam)
gam.check(m1.TN.gam, rep=1000)
draw(m1.TN.gam)
concrvity(m1.TN.gam)


```
T2 model
```{r}
m2.TN.gam=gam(log10(TN..mg.L) ~ Treatment + s(plant.mass..g,by=Treatment, k=4, bs="ps"), subset = Time.point=="T2", data = DOC.df2, method = "REML")
summary(m2.TN.gam)
anova.gam(m2.TN.gam)
gam.check(m2.TN.gam, rep=1000)
draw(m2.TN.gam)
concrvity(m2.TN.gam)


```

T3 model
```{r}
m3.TN.gam=gam(log10(TN..mg.L) ~ Treatment + s(plant.mass..g,by=Treatment, k=4, bs="ps"), subset = Time.point=="T3", data = DOC.df2, method = "REML")
summary(m3.TN.gam)
anova.gam(m3.TN.gam)
gam.check(m3.TN.gam, rep=1000)
draw(m3.TN.gam)
concrvity(m3.TN.gam)


```

T4 model
```{r}
m4.TN.gam=gam(log10(TN..mg.L) ~ Treatment + s(plant.mass..g,by=Treatment, k=4, bs="ps"), subset = Time.point=="T4", data = DOC.df2, method = "REML")
summary(m4.TN.gam)
anova.gam(m4.TN.gam)
gam.check(m4.TN.gam, rep=1000)
draw(m4.TN.gam)
concrvity(m4.TN.gam)


```

UVBIO files
```{r}
##### grab files in a list
UVBio.files <- list.files(path="/Users/Cody1/Desktop/GitHub/Pyromania/data/UVBIO", pattern = "csv$", full.names = T)
UVBio.files

##### what are the file names, sans extensions using package 'tools'
file.names<-file_path_sans_ext(list.files(path="/Users/Cody1/Desktop/GitHub/Pyromania/data/UVBIO", pattern = "csv$", full.names = F))
file.names


############ formatting all data in for loop
  for(i in 1:length(UVBio.files))
    {
  data<-read.csv(UVBio.files[i], sep=",") #skip 13 rows where standards are
  data<-data[,c(1,3,4)] # removed columns we don't need
  data$File<-UVBio.files[i]
  colnames(data)<-c("Tank", "DOC..mg.L", "TN..mg.L", "File") 
  data$Tank<-as.character(data$Tank)
  data$Treatment<-IDs$Treatment # add treatment info
  data$plant.mass..g<-IDs$plant.mass..g # add plant biomass info
  make.names(assign(paste(file.names[i], sep=""), data)) # make the file name the name of new df for loop df
  }
########## this is the end of the loop

ls() #see all dfs you've made, the above will be df matching their file names

#Combine files from loop to single df
UVBIO.df<-rbind(DOC_NoUV_T1, DOC_UV_T1, DOC_NoUV_T2, DOC_UV_T2, DOC_NoUV_T3, DOC_UV_T3)

# make new factors for filtered (F/NF) and UV (+/-)
# ** caution! Make sure this matches sheet layout after the loop!

UVBIO.df$Filter.Trt<-rep(c("Filtered", "Unfiltered"), each=30) # repeat 30 times
UVBIO.df$UV.Trt<-rep(c("NoUV", "UV"), each=60) # repeat 60 times

# check tank names in DF and make sure they are what you think they are (i.e 1-30 but w/ weird names)
Tank.fix<-rep(c(1:30), times=12) # repeat 12 times to fix all the strange tank names 

#compare tank IDS
df.test<-cbind(Tank.fix, UVBIO.df$Tank) # yep!

UVBIO.df$Tank<-as.factor(Tank.fix) #replace old tank names

# extract out the file names
#Extract file
UVBIO.df$File=sapply(strsplit(UVBIO.df$File, "/"), `[`, 9)
#take out redundancies
UVBIO.df$DOC..mg.L<-gsub("NPOC:","", UVBIO.df$DOC..mg.L)
UVBIO.df$DOC..mg.L<- gsub("mg/L", "", UVBIO.df$DOC..mg.L)
UVBIO.df$TN..mg.L<-gsub("TN:","", UVBIO.df$TN..mg.L)
UVBIO.df$TN..mg.L<- gsub("mg/L", "", UVBIO.df$TN..mg.L)

#convert DOC and TN to numeric
UVBIO.df$DOC..mg.L<-as.numeric(as.character(UVBIO.df$DOC..mg.L))
UVBIO.df$TN..mg.L<- as.numeric(as.character(UVBIO.df$TN..mg.L))

# add a time point
# if equals DOC_UV_T1_111721 then, T1, if not then "nothing"
UVBIO.df$Time.point<- as.factor(ifelse(UVBIO.df$File=="DOC_UV_T1.csv", "T1",
  ifelse(UVBIO.df$File=="DOC_NoUV_T1.csv", "T1",
          ifelse(UVBIO.df$File=="DOC_UV_T2.csv", "T2",
                ifelse(UVBIO.df$File=="DOC_NoUV_T2.csv", "T2", "T3")))))

#rearrange
UVBIO.df<- UVBIO.df %>% 
  dplyr::select(File, Time.point, Treatment, Filter.Trt, UV.Trt, Tank, plant.mass..g, DOC..mg.L, TN..mg.L)


# make levels for plotting and stats
# add in starting DOC and TN
# original data is... DOC_T0, DOC_T1, DOC_T2, DOC_T3, DOC_T4 -- but no need for DOC T0 and T4
# repeat 4x to account for the 4 treatments in each tank
DOC_T1.start<-rep(DOC_T1$DOC..mg.L, times=4)
DOC_T2.start<-rep(DOC_T2$DOC..mg.L, times=4)
DOC_T3.start<-rep(DOC_T3$DOC..mg.L, times=4)

TN_T1.start<-rep(DOC_T1$TN..mg.L, times=4)
TN_T2.start<-rep(DOC_T2$TN..mg.L, times=4)
TN_T3.start<-rep(DOC_T3$TN..mg.L, times=4)

# compile and add in as a new column
UVBIO.df$Start.DOC<-c(DOC_T1.start, DOC_T2.start, DOC_T3.start)
UVBIO.df$Start.TN<-c(TN_T1.start, TN_T2.start, TN_T3.start)

UVBIO.df$Burn.Filt.UV<-interaction(UVBIO.df$Treatment, UVBIO.df$Filter.Trt, UVBIO.df$UV.Trt)
UVBIO.df$Filt.UV<-interaction(UVBIO.df$Filter.Trt, UVBIO.df$UV.Trt)

```
SUVA 
SUVA calculations
Time point 0
```{r}
#load file and rearrange
<<<<<<< HEAD
suva.t0.df=read.csv("/Users/Cody1/Desktop/GitHub/Pyromania/data/EEMs/Results_T0.csv")
=======
suva.t0.df=read.csv("/Users/tobyspiegel2/Desktop/GitHub/Pyromania/data/EEMs/Results_T0.csv")
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
suva.t0.df$Sample.Name=gsub("\\..*", "", suva.t0.df$Sample.Name) 
names(suva.t0.df)[1]="Tank"
suva.t0.df=suva.t0.df[-1,]
suva.t0.df$Tank=gsub("T", "", suva.t0.df$Tank) 
suva.t0.df$Tank=as.numeric(suva.t0.df$Tank)
suva.t0.df=suva.t0.df[order(suva.t0.df$Tank),]

#add important info
suva.t0.df$DOC..mg.L=DOC_T0$DOC..mg.L
suva.t0.df$Time.point="T0"
suva.t0.df$Treatment=IDs$Treatment
suva.t0.df$plant.mass..g=IDs$plant.mass..g

#calculate SUVA
suva.t0.df$SUVA=(suva.t0.df[suva.t0.df$Tank,]$abs254/suva.t0.df[suva.t0.df$Tank,]$DOC..mg.L)*100
```

Time point 1
```{r}
#load file and rearrange
library(dplyr)
<<<<<<< HEAD
suva.t1.df=read.csv("/Users/Cody1/Desktop/GitHub/Pyromania/data/EEMs/Results_T1.csv")
=======
suva.t1.df=read.csv("/Users/tobyspiegel2/Desktop/GitHub/Pyromania/data/EEMs/Results_T1.csv")
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
suva.t1.df=suva.t1.df[-1,]
suva.t1.df=suva.t1.df[-1,]
names(suva.t1.df)[1]="Tank"
suva.t1.df$Tank=gsub("TA", "", suva.t1.df$Tank) 
suva.t1.df$Tank=sub("(.{2})(.*)", "\\1.\\2", suva.t1.df$Tank)
suva.t1.df$Tank=sub("\\..*", "", suva.t1.df$Tank)
suva.t1.df$Tank=as.numeric(suva.t1.df$Tank)
suva.t1.df=suva.t1.df[order(suva.t1.df$Tank),]

#add important info
suva.t1.df$DOC..mg.L=DOC_T1$DOC..mg.L
suva.t1.df$Time.point="T1"
suva.t1.df$Treatment=IDs$Treatment
suva.t1.df$plant.mass..g=IDs$plant.mass..g

#calculate SUVA
suva.t1.df$SUVA=(suva.t1.df[suva.t1.df$Tank,]$abs254/suva.t1.df[suva.t1.df$Tank,]$DOC..mg.L)*100
```

Time point 2
```{r}
#load file and rearrange
<<<<<<< HEAD
suva.t2.df=read.csv("/Users/Cody1/Desktop/GitHub/Pyromania/data/EEMs/Results_T2_d1A.edit.csv")
=======
suva.t2.df=read.csv("/Users/tobyspiegel2/Desktop/GitHub/Pyromania/data/EEMs/Results_T2_d1A.edit.csv")
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
names(suva.t2.df)[1]="Tank"
suva.t2.df$Tank=gsub("TA", "", suva.t2.df$Tank) 
suva.t2.df$Tank=sub("(.{2})(.*)", "\\1.\\2", suva.t2.df$Tank)
suva.t2.df$Tank=sub("\\..*", "", suva.t2.df$Tank)
suva.t2.df$Tank=as.numeric(suva.t2.df$Tank)
suva.t2.df=suva.t2.df[order(suva.t2.df$Tank),]

#add important info
suva.t2.df$DOC..mg.L=DOC_T2$DOC..mg.L
suva.t2.df$Time.point="T2"
suva.t2.df$Treatment=IDs$Treatment
suva.t2.df$plant.mass..g=IDs$plant.mass..g

#calculate SUVA
suva.t2.df$SUVA=(suva.t2.df[suva.t2.df$Tank,]$abs254/suva.t2.df[suva.t2.df$Tank,]$DOC..mg.L)*100

```

Time point 3
```{r}
<<<<<<< HEAD
suva.t3.df=read.csv("/Users/Cody1/Desktop/GitHub/Pyromania/data/EEMs/Results_T3.csv")
=======
suva.t3.df=read.csv("/Users/tobyspiegel2/Desktop/GitHub/Pyromania/data/EEMs/Results_T3.csv")
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
names(suva.t3.df)[1]="Tank"
suva.t3.df$Tank=gsub("TA", "", suva.t3.df$Tank) 
suva.t3.df$Tank=sub("(.{2})(.*)", "\\1.\\2", suva.t3.df$Tank)
suva.t3.df$Tank=sub("\\..*", "", suva.t3.df$Tank)
suva.t3.df$Tank=as.numeric(suva.t3.df$Tank)
suva.t3.df=suva.t3.df[order(suva.t3.df$Tank),]


#add important info
suva.t3.df$DOC..mg.L=DOC_T3$DOC..mg.L
suva.t3.df$Time.point="T3"
suva.t3.df$Treatment=IDs$Treatment
suva.t3.df$plant.mass..g=IDs$plant.mass..g
class(suva.t3.df$DOC..mg.L)

#calculate SUVA
suva.t3.df$SUVA=(suva.t3.df[suva.t3.df$Tank,]$abs254/suva.t3.df[suva.t3.df$Tank,]$DOC..mg.L)*100

```

Time point 4
```{r}
<<<<<<< HEAD
suva.t4.df=read.csv("/Users/Cody1/Desktop/GitHub/Pyromania/data/EEMs/Results_T4.csv")
=======
suva.t4.df=read.csv("/Users/tobyspiegel2/Desktop/GitHub/Pyromania/data/EEMs/Results_T4.csv")
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
names(suva.t4.df)[1]="Tank"
suva.t4.df$Tank=gsub("TA", "", suva.t4.df$Tank) 
suva.t4.df$Tank=sub("(.{2})(.*)", "\\1.\\2", suva.t4.df$Tank)
suva.t4.df$Tank=sub("\\..*", "", suva.t4.df$Tank)
suva.t4.df$Tank=as.numeric(suva.t4.df$Tank)
suva.t4.df=suva.t4.df[order(suva.t4.df$Tank),]

#add important info
suva.t4.df$DOC..mg.L=DOC_T4$DOC..mg.L
suva.t4.df$Time.point="T4"
suva.t4.df$Treatment=IDs$Treatment
suva.t4.df$plant.mass..g=IDs$plant.mass..g

#calculate SUVA
suva.t4.df$SUVA=(suva.t4.df[suva.t4.df$Tank,]$abs254/suva.t4.df[suva.t4.df$Tank,]$DOC..mg.L)*100

```

Combine all SUVA time points
```{r}
suva.df=rbind(suva.t0.df, suva.t1.df, suva.t2.df, suva.t3.df, suva.t4.df)


```


Maybe add categories to SUVA
```{r}
library(dplyr)

#5, 125, 225g subsets

suva.subset.df=suva.df %>% 
  filter((plant.mass..g == "5" | plant.mass..g == "125" | plant.mass..g == "225"))

suva.subset.df$Level=as.factor(ifelse(suva.subset.df$plant.mass..g=="5", "Low",
         ifelse (suva.subset.df$plant.mass..g=="125", "Medium",
           ifelse (suva.subset.df$plant.mass..g=="225", "High", "High"))))

library(gdata)

suva.subset.df=suva.subset.df[order(suva.subset.df$Level),]



```



Other indices
```{r}
# Time 0
<<<<<<< HEAD
index.T0.df=read.csv("/Users/Cody1/Desktop/My_pyromania/EEMS/Indexresults/T0_indices.csv")
=======
index.T0.df=read.csv("/Users/tobyspiegel2/Desktop/My_pyromania/EEMS/Indexresults/T0_indices.csv")
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
index.T0.df$Time.point="T0"
names(index.T0.df)[6]="Humification.Index"
index.T0.df=index.T0.df[-c(7:11,13,14)]


#Time 1
<<<<<<< HEAD
index.T1.df=read.csv("/Users/Cody1/Desktop/My_pyromania/EEMS/Indexresults/T1_indices.csv")
=======
index.T1.df=read.csv("/Users/tobyspiegel2/Desktop/My_pyromania/EEMS/Indexresults/T1_indices.csv")
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
index.T1.df$Time.point="T1"
index.T1.df=index.T1.df[-c(7:11,13,14)]

#Time 2
<<<<<<< HEAD
index.T2.df=read.csv("/Users/Cody1/Desktop/My_pyromania/EEMS/Indexresults/T2_indices.csv")
=======
index.T2.df=read.csv("/Users/tobyspiegel2/Desktop/My_pyromania/EEMS/Indexresults/T2_indices.csv")
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
index.T2.df$Time.point="T2"
index.T2.df=index.T2.df[-c(7:11,13,14)]

#Time 3
<<<<<<< HEAD
index.T3.df=read.csv("/Users/Cody1/Desktop/My_pyromania/EEMS/Indexresults/T3_indices.csv")
=======
index.T3.df=read.csv("/Users/tobyspiegel2/Desktop/My_pyromania/EEMS/Indexresults/T3_indices.csv")
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
index.T3.df$Time.point="T3"
index.T3.df=index.T3.df[-c(7:11,13,14)]


#Time 4
<<<<<<< HEAD
index.T4.df=read.csv("/Users/Cody1/Desktop/My_pyromania/EEMS/Indexresults/T4_indicies.csv")
=======
index.T4.df=read.csv("/Users/tobyspiegel2/Desktop/My_pyromania/EEMS/Indexresults/T4_indicies.csv")
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
index.T4.df$Time.point="T4"
index.T4.df=index.T4.df[-c(7:11,13,14)]

#combine all index time points
index.df=rbind(index.T0.df, index.T1.df, index.T2.df, index.T3.df, index.T4.df)


```





Calculate photo, microbe, and total degradation:
```{r}
#+UV/+Bio=A
#-UV/+Bio=B
#+UV/-Bio=C
#-UV/-Bio (NoUV,Filtered)=D

#This is calculating % decrease ((control-treatment)/control)*100

#photodegradation

photo.df=data.frame(UVBIO.df[UVBIO.df$Filt.UV=="Filtered.UV",]$Time.point,
                    UVBIO.df[UVBIO.df$Filt.UV=="Filtered.UV",]$Start.DOC,
                    UVBIO.df[UVBIO.df$Filt.UV=="Filtered.UV",]$Start.TN,
                    UVBIO.df[UVBIO.df$Filt.UV=="Filtered.UV",]$Treatment,
                    UVBIO.df[UVBIO.df$Filt.UV=="Filtered.UV",]$Tank,
                    UVBIO.df[UVBIO.df$Filt.UV=="Filtered.UV",]$plant.mass..g,
                    UVBIO.df[UVBIO.df$Filt.UV=="Filtered.UV",]$DOC..mg.L-UVBIO.df[UVBIO.df$Filt.UV=="Filtered.UV",]$Start.DOC,
                    100*(UVBIO.df[UVBIO.df$Filt.UV=="Filtered.UV",]$DOC..mg.L-UVBIO.df[UVBIO.df$Filt.UV=="Filtered.UV",]$Start.DOC)/((UVBIO.df[UVBIO.df$Filt.UV=="Filtered.UV",]$Start.DOC)), 
                     UVBIO.df[UVBIO.df$Filt.UV=="Filtered.UV",]$TN..mg.L-UVBIO.df[UVBIO.df$Filt.UV=="Filtered.UV",]$Start.TN,
                    100*(UVBIO.df[UVBIO.df$Filt.UV=="Filtered.UV",]$TN..mg.L-UVBIO.df[UVBIO.df$Filt.UV=="Filtered.UV",]$Start.TN)/((UVBIO.df[UVBIO.df$Filt.UV=="Filtered.UV",]$Start.TN)),
                     suva.df[suva.df$Time.point=="T1",]$SUVA,
                     suva.df[suva.df$Time.point=="T1",]$Spectral.Slope.Ratio,
                    index.df[index.df$Time.point=="T1",]$Humification.Index)

photo.df$effect="UV-only"
names(photo.df)[1]="Time.point"
names(photo.df)[2]="Start.DOC"
names(photo.df)[3]="Start.TN"
names(photo.df)[4]="Treatment"
names(photo.df)[5]="Tank"
names(photo.df)[6]="plant.mass..g"
names(photo.df)[7]="abs.change.DOC"
names(photo.df)[8]="percent.change.DOC"
names(photo.df)[9]="abs.change.TN"
names(photo.df)[10]="percent.change.TN"
names(photo.df)[11]="SUVA"
names(photo.df)[12]="Spectral.slope"
names(photo.df)[13]="HIX"
photo.df$mass.remaining.DOC=100-abs(photo.df[photo.df$effect=="UV-only",]$percent.change.DOC)
photo.df$mass.remaining.TN=100-abs(photo.df[photo.df$effect=="UV-only",]$percent.change.TN)


#Microbe decomp
microbe.df=data.frame(UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.NoUV",]$Time.point,
                      UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.NoUV",]$Start.DOC,
                      UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.NoUV",]$Start.TN,
                      UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.NoUV",]$Treatment,
                      UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.NoUV",]$Tank,
                      UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.NoUV",]$plant.mass..g,
                    UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.NoUV",]$DOC..mg.L- UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.NoUV",]$Start.DOC,
                    100*(UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.NoUV",]$DOC..mg.L-UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.NoUV",]$Start.DOC)/(( UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.NoUV",]$Start.DOC)),
                     UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.NoUV",]$TN..mg.L-UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.NoUV",]$Start.TN,
                    100*(UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.NoUV",]$TN..mg.L-UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.NoUV",]$Start.TN)/((UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.NoUV",]$Start.TN)),
                     suva.df[suva.df$Time.point=="T1",]$SUVA,
                    suva.df[suva.df$Time.point=="T1",]$Spectral.Slope.Ratio,
                    index.df[index.df$Time.point=="T1",]$Humification.Index)

microbe.df$effect="microbes-only"
names(microbe.df)[1]="Time.point"
names(microbe.df)[2]="Start.DOC"
names(microbe.df)[3]="Start.TN"
names(microbe.df)[4]="Treatment"
names(microbe.df)[5]="Tank"
names(microbe.df)[6]="plant.mass..g"
names(microbe.df)[7]="abs.change.DOC"
names(microbe.df)[8]="percent.change.DOC"
names(microbe.df)[9]="abs.change.TN"
names(microbe.df)[10]="percent.change.TN"
names(microbe.df)[11]="SUVA"
names(microbe.df)[12]="Spectral.slope"
names(microbe.df)[13]="HIX"
microbe.df$mass.remaining.DOC=100-abs(microbe.df[microbe.df$effect=="microbes-only",]$percent.change.DOC)
microbe.df$mass.remaining.TN=100-abs(microbe.df[microbe.df$effect=="microbes-only",]$percent.change.TN)


#Total decomp= A
total.df=data.frame(UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$Time.point,
                    UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$Start.DOC,
                    UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$Start.TN,
                    UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$Treatment,
                    UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$Tank,
                    UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$plant.mass..g,
                    UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$DOC..mg.L-UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$Start.DOC,
                     100*(UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$DOC..mg.L-UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$Start.DOC)/(abs(UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$Start.DOC)),
                      UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$TN..mg.L-UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$Start.TN,
                     100*(UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$TN..mg.L-UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$Start.TN)/(abs(UVBIO.df[UVBIO.df$Filt.UV=="Unfiltered.UV",]$Start.TN)),
                     suva.df[suva.df$Time.point=="T1",]$SUVA,
                    suva.df[suva.df$Time.point=="T1",]$Spectral.Slope.Ratio,
                     index.df[index.df$Time.point=="T1",]$Humification.Index)
                     
total.df$effect="UV and microbes"
names(total.df)[1]="Time.point"
names(total.df)[2]="Start.DOC"
names(total.df)[3]="Start.TN"
names(total.df)[4]="Treatment"
names(total.df)[5]="Tank"
names(total.df)[6]="plant.mass..g"
names(total.df)[7]="abs.change.DOC"
names(total.df)[8]="percent.change.DOC"
names(total.df)[9]="abs.change.TN"
names(total.df)[10]="percent.change.TN"
names(total.df)[11]="SUVA"
names(total.df)[12]="Spectral.slope"
names(total.df)[13]="HIX"
total.df$mass.remaining.DOC=100-abs(total.df[total.df$effect=="UV and microbes",]$percent.change.DOC)
total.df$mass.remaining.TN=100-abs(total.df[total.df$effect=="UV and microbes",]$percent.change.TN)

#merge degra.df time points into one dataframe


degra.df=rbind(photo.df, microbe.df, total.df)
degra.df=pivot_longer(degra.df, cols=c("effect"), names_to="effect", values_to="Type")

#make sure we have factors
degra.df=degra.df%>%
    mutate_if(is.character,as.factor)

<<<<<<< HEAD


```

UVBIO DOC with just Time-1 % change
```{r}
library(mgcv)
# The palette with grey:
cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")


UVBIO.T1.DOC<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=plant.mass..g, y=percent.change.DOC, color=Type, linetype=Treatment))+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("mediumblue", "indianred1", "yellow2")) + 
  scale_fill_manual(values = c("mediumblue", "indianred1", "yellow2")) +
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Type), alpha=0.2) +
  ylab(expression(paste("Change DOC (%)")))+
  xlab("Plant mass (g)")+
  xlim(0,300)+
  ggtitle("Time-1")+
  Fig.formatting

UVBIO.T1.DOC

Fig.formatting.degra<-(theme_classic()) +
  theme(text=element_text(size=14),
        legend.text.align = 0,
        panel.border = element_rect(fill=NA, colour = "black", size=1),
        aspect.ratio=1, 
        axis.ticks.length=unit(0.25, "cm"),
        axis.text.y=element_text(
        margin=unit(c(0.5, 0.5,0.5,0.5), "cm"), colour="black", size=12), 
        axis.text.x=element_text(
        margin=unit(c(0.5,0.5,0.5,0.5), "cm"), colour="black", size=12)) +
  theme(legend.key.size = unit(0.4, "cm")) +
  theme(aspect.ratio=1.3) +
  theme(panel.spacing=unit(c(0), "pt"))
=======
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0

UVBIO.T1.effect<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=plant.mass..g, y=percent.change.DOC, color=Treatment, linetype=Treatment))+
  facet_grid(~Type)+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Treatment), alpha=0.2) +
  ylab(expression(paste("Change DOC (%)")))+
  xlab("Plant mass (g)")+
  Fig.formatting.degra

<<<<<<< HEAD
UVBIO.T1.effect
```

UVBIO with just T1 ABS change
```{r}
cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")


UVBIO.T1.DOC.abs<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=plant.mass..g, y=abs.change.DOC, color=Type, linetype=Treatment))+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
 scale_color_manual(values= c("mediumblue", "indianred1", "yellow2")) + 
  scale_fill_manual(values = c("mediumblue", "indianred1", "yellow2")) +
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Treatment), alpha=0.2) +
ylab(expression(paste(Delta~"DOC (mg/L)")))+
  xlab("Plant mass (g)")+
  theme(legend.title = "none")+
  ggtitle("Time-1")+
  xlim(0,300)+
  Fig.formatting


UVBIO.T1.effect.abs<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=plant.mass..g, y=abs.change.DOC, color=Treatment, linetype=Treatment))+
  facet_grid(~Type)+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
   scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Treatment), alpha=0.2) +
    ylab(expression(paste(Delta~"DOC mg"~L^-1)))+
  xlab("Plant mass (g)")+
  Fig.formatting.degra


```

UVBIO with just T1 SUVA
```{r}
cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")


UVBIO.T1.DOC.suva<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=SUVA, y=percent.change.DOC, color=Type, linetype=Treatment))+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("mediumblue", "indianred1", "yellow2")) + 
  scale_fill_manual(values = c("mediumblue", "indianred1", "yellow2")) +
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Type), alpha=0.2) +
  ylab(expression(paste("Change DOC (%)")))+
  xlab(bquote("SUVA (mgC" ~ L^-1~m^-1*')'))+
  ggtitle("Time-1")+
    xlim(1.2,3)+
  Fig.formatting


Fig.formatting.degra<-(theme_classic()) +
  theme(text=element_text(size=14),
        legend.text.align = 0,
        panel.border = element_rect(fill=NA, colour = "black", size=1),
        aspect.ratio=1, 
        axis.ticks.length=unit(0.25, "cm"),
        axis.text.y=element_text(
        margin=unit(c(0.5, 0.5,0.5,0.5), "cm"), colour="black", size=12), 
        axis.text.x=element_text(
        margin=unit(c(0.5,0.5,0.5,0.5), "cm"), colour="black", size=12)) +
  theme(legend.key.size = unit(0.4, "cm")) +
  theme(aspect.ratio=1.3) +
  theme(panel.spacing=unit(c(0), "pt"))

UVBIO.T1.effect.suva<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=SUVA, y=percent.change.DOC, color=Type, linetype=Treatment))+
  facet_grid(~Type)+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("mediumblue", "indianred1", "yellow2")) + 
  scale_fill_manual(values = c("mediumblue", "indianred1", "yellow2")) +
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Type), alpha=0.2) +
    ylab(expression(paste("Change DOC (%)")))+
 xlab(bquote("SUVA (mgC" ~ L^-1~m^-1*')'))+
    xlim(1.2,3)+
  Fig.formatting.degra


```


UVBIO with just T1 spectral slope
```{r}
cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")


UVBIO.T1.DOC.spec<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=Spectral.slope, y=percent.change.DOC, color=Effect, linetype=Treatment))+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("darkgreen", "darkorange2", "darkorchid")) + 
  scale_fill_manual(values = c("darkgreen", "darkorange2", "darkorchid")) +
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Effect), alpha=0.2) +
 ylab("Change DOC (%)")+
  ggtitle("Time-1")+
  xlab(expression(S[italic(R)]))+
  theme(legend.title = "none")+
  Fig.formatting

=======
```

UVBIO DOC with just Time-1 % change
```{r}
library(mgcv)
# The palette with grey:
cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")


UVBIO.T1.DOC<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=plant.mass..g, y=percent.change.DOC, color=Type, linetype=Treatment))+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("mediumblue", "indianred1", "yellow2")) + 
  scale_fill_manual(values = c("mediumblue", "indianred1", "yellow2")) +
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Type), alpha=0.2) +
  ylab(expression(paste("Change DOC (%)")))+
  xlab("Plant mass (g)")+
  xlim(0,300)+
  ggtitle("Time-1")+
  Fig.formatting

UVBIO.T1.DOC
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0

Fig.formatting.degra<-(theme_classic()) +
  theme(text=element_text(size=14),
        legend.text.align = 0,
        panel.border = element_rect(fill=NA, colour = "black", size=1),
        aspect.ratio=1, 
        axis.ticks.length=unit(0.25, "cm"),
        axis.text.y=element_text(
        margin=unit(c(0.5, 0.5,0.5,0.5), "cm"), colour="black", size=12), 
        axis.text.x=element_text(
        margin=unit(c(0.5,0.5,0.5,0.5), "cm"), colour="black", size=12)) +
  theme(legend.key.size = unit(0.4, "cm")) +
  theme(aspect.ratio=1.3) +
  theme(panel.spacing=unit(c(0), "pt"))

<<<<<<< HEAD
UVBIO.T1.effect.spec<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=Spectral.slope, y=percent.change.DOC, color=Effect, linetype=Treatment))+
  facet_grid(~Effect)+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("darkgreen", "darkorange2", "darkorchid")) + 
  scale_fill_manual(values = c("darkgreen", "darkorange2", "darkorchid")) +
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Effect), alpha=0.2) +
   ylab("Change DOC (%)")+
  xlab(expression(S[italic(R)]))+
  Fig.formatting.degra



```



UVBIO with just T1 HIX
```{r}
cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")


UVBIO.T1.DOC.hix<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=HIX, y=percent.change.DOC, color=Effect, linetype=Treatment))+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("darkgreen", "darkorange2", "darkorchid")) + 
  scale_fill_manual(values = c("darkgreen", "darkorange2", "darkorchid")) +
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Effect), alpha=0.2) +
  ylab(expression(paste("Change DOC (%)")))+
  xlab("HIX")+
=======
UVBIO.T1.effect<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=plant.mass..g, y=percent.change.DOC, color=Treatment, linetype=Treatment))+
  facet_grid(~Type)+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Treatment), alpha=0.2) +
  ylab(expression(paste("Change DOC (%)")))+
  xlab("Plant mass (g)")+
  Fig.formatting.degra

UVBIO.T1.effect
```

UVBIO with just T1 ABS change
```{r}
cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")


UVBIO.T1.DOC.abs<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=plant.mass..g, y=abs.change.DOC, color=Type, linetype=Treatment))+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
 scale_color_manual(values= c("mediumblue", "indianred1", "yellow2")) + 
  scale_fill_manual(values = c("mediumblue", "indianred1", "yellow2")) +
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Treatment), alpha=0.2) +
ylab(expression(paste(Delta~"DOC (mg/L)")))+
  xlab("Plant mass (g)")+
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
  theme(legend.title = "none")+
  ggtitle("Time-1")+
  xlim(0,300)+
  Fig.formatting

<<<<<<< HEAD


UVBIO.T1.effect.hix<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=HIX, y=percent.change.DOC, color=Effect, linetype=Treatment))+
  facet_grid(~Effect)+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("darkgreen", "darkorange2", "darkorchid")) + 
  scale_fill_manual(values = c("darkgreen", "darkorange2", "darkorchid")) +
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Effect), alpha=0.2) +
    ylab(expression(paste("Change DOC (%)")))+
  xlab("HIX")+
  Fig.formatting.degra



```

=======

UVBIO.T1.effect.abs<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=plant.mass..g, y=abs.change.DOC, color=Treatment, linetype=Treatment))+
  facet_grid(~Type)+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
   scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Treatment), alpha=0.2) +
    ylab(expression(paste(Delta~"DOC mg"~L^-1)))+
  xlab("Plant mass (g)")+
  Fig.formatting.degra


```

UVBIO with just T1 SUVA
```{r}
cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")


UVBIO.T1.DOC.suva<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=SUVA, y=percent.change.DOC, color=Type, linetype=Treatment))+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("mediumblue", "indianred1", "yellow2")) + 
  scale_fill_manual(values = c("mediumblue", "indianred1", "yellow2")) +
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Type), alpha=0.2) +
  ylab(expression(paste("Change DOC (%)")))+
  xlab(bquote("SUVA (mgC" ~ L^-1~m^-1*')'))+
  ggtitle("Time-1")+
    xlim(1.2,3)+
  Fig.formatting


Fig.formatting.degra<-(theme_classic()) +
  theme(text=element_text(size=14),
        legend.text.align = 0,
        panel.border = element_rect(fill=NA, colour = "black", size=1),
        aspect.ratio=1, 
        axis.ticks.length=unit(0.25, "cm"),
        axis.text.y=element_text(
        margin=unit(c(0.5, 0.5,0.5,0.5), "cm"), colour="black", size=12), 
        axis.text.x=element_text(
        margin=unit(c(0.5,0.5,0.5,0.5), "cm"), colour="black", size=12)) +
  theme(legend.key.size = unit(0.4, "cm")) +
  theme(aspect.ratio=1.3) +
  theme(panel.spacing=unit(c(0), "pt"))

UVBIO.T1.effect.suva<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=SUVA, y=percent.change.DOC, color=Type, linetype=Treatment))+
  facet_grid(~Type)+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("mediumblue", "indianred1", "yellow2")) + 
  scale_fill_manual(values = c("mediumblue", "indianred1", "yellow2")) +
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Type), alpha=0.2) +
    ylab(expression(paste("Change DOC (%)")))+
 xlab(bquote("SUVA (mgC" ~ L^-1~m^-1*')'))+
    xlim(1.2,3)+
  Fig.formatting.degra


```


UVBIO with just T1 spectral slope
```{r}
cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")


UVBIO.T1.DOC.spec<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=Spectral.slope, y=percent.change.DOC, color=Effect, linetype=Treatment))+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("darkgreen", "darkorange2", "darkorchid")) + 
  scale_fill_manual(values = c("darkgreen", "darkorange2", "darkorchid")) +
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Effect), alpha=0.2) +
 ylab("Change DOC (%)")+
  ggtitle("Time-1")+
  xlab(expression(S[italic(R)]))+
  theme(legend.title = "none")+
  Fig.formatting
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0

UVBIO analysis

<<<<<<< HEAD
T1 model
```{r}
library(mgcv)
library(gratia)

degra.df=rbind(microbe.df, total.df)
degra.df=pivot_longer(degra.df, cols=c("type"), names_to="type", values_to="Effect")

degra.df$SUVA=suva.df[suva.df$Time.point,]$SUVA

#make sure we have factors
degra.df=degra.df%>%
    mutate_if(is.character,as.factor)


=======

UVBIO.T1.effect.spec<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=Spectral.slope, y=percent.change.DOC, color=Effect, linetype=Treatment))+
  facet_grid(~Effect)+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("darkgreen", "darkorange2", "darkorchid")) + 
  scale_fill_manual(values = c("darkgreen", "darkorange2", "darkorchid")) +
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Effect), alpha=0.2) +
   ylab("Change DOC (%)")+
  xlab(expression(S[italic(R)]))+
  Fig.formatting.degra



```



UVBIO with just T1 HIX
```{r}
cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")


UVBIO.T1.DOC.hix<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=HIX, y=percent.change.DOC, color=Effect, linetype=Treatment))+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("darkgreen", "darkorange2", "darkorchid")) + 
  scale_fill_manual(values = c("darkgreen", "darkorange2", "darkorchid")) +
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Effect), alpha=0.2) +
  ylab(expression(paste("Change DOC (%)")))+
  xlab("HIX")+
  theme(legend.title = "none")+
  Fig.formatting



UVBIO.T1.effect.hix<-ggplot(degra.df[(degra.df$Time.point=="T1"),], aes(x=HIX, y=percent.change.DOC, color=Effect, linetype=Treatment))+
  facet_grid(~Effect)+
 guides(linetype = guide_legend(override.aes = list(color = "black")))+
  geom_point(alpha=0.4) +
  scale_color_manual(values= c("darkgreen", "darkorange2", "darkorchid")) + 
  scale_fill_manual(values = c("darkgreen", "darkorange2", "darkorchid")) +
  geom_smooth(method=gam, formula= y~s(x), aes(fill=Effect), alpha=0.2) +
    ylab(expression(paste("Change DOC (%)")))+
  xlab("HIX")+
  Fig.formatting.degra



```


UVBIO analysis

T1 model
```{r}
library(mgcv)
library(gratia)

degra.df=rbind(microbe.df, total.df)
degra.df=pivot_longer(degra.df, cols=c("type"), names_to="type", values_to="Effect")

degra.df$SUVA=suva.df[suva.df$Time.point,]$SUVA

#make sure we have factors
degra.df=degra.df%>%
    mutate_if(is.character,as.factor)


>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
#####
uv.m1.gam.perc=gam(percent.change.DOC ~ Type*Treatment +s(plant.mass..g, by= Type), data=degra.df, method="REML", select= T, subset= Time.point=="T1")
summary(uv.m1.gam.perc)
anova(uv.m1.gam.perc)
draw(uv.m1.gam.perc)
gam.check(uv.m1.gam.perc, rep=1000)

decompose(degra.df)

uv.m1.gam.abs=gam(abs.change.DOC ~ Type*Treatment +s(plant.mass..g, by= Type), data=degra.df, method="REML", select= T, subset= Time.point=="T1")
summary(uv.m1.gam.abs)
anova(uv.m1.gam.abs)
draw(uv.m1.gam.abs)
gam.check(uv.m1.gam.abs, rep=1000)

uv.m1.gam.spec=gam(percent.change.DOC ~ Type*Treatment +s(Spectral.slope, by= Type), data=degra.df, method="REML", select= T, subset= Time.point=="T1")
summary(uv.m1.gam.spec)
anova(uv.m1.gam.spec)
gam.check(uv.m1.gam.spec, rep=1000)
draw(uv.m1.gam.spec)
TukeyHSD(anova(uv.m1.gam))

library(multcomp)
glht(uv.m1.gam,linfct=mcp(effect="Tukey"))

draw(uv.m1.gam)
concrvity(uv.m1.gam)
gam.check(uv.m1.gam, rep=1000)
######
```

Sage and Willow Decomp data
```{r}
#load files and rearrange 
decomp.df=read.csv("/Users/Cody1/Desktop/GitHub/Pyromania/data/decomp.weights.csv")
names(decomp.df)[8]= "change"
names(decomp.df)[9]= "percent.change"
decomp.df=decomp.df[-c(61:82), ] 



#make a duplicate dataframe and convert % to decimal
decomp2.df=data.frame(decomp.df)

#create a sage dataframe
sage.df=data.frame(decomp.df[decomp.df$plant.type=="sage",]$treatment,
                    decomp.df[decomp.df$plant.type=="sage",]$tank,
                    decomp.df[decomp.df$plant.type=="sage",]$plant.specific.mass..g,
                    decomp.df[decomp.df$plant.type=="sage",]$dry.mass..g.bag.corrected-decomp.df[decomp.df$plant.type=="sage",]$plant.specific.mass..g,
                     (100*(decomp.df[decomp.df$plant.type=="sage",]$dry.mass..g.bag.corrected-decomp.df[decomp.df$plant.type=="sage",]$plant.specific.mass..g)/(decomp.df[decomp.df$plant.type=="sage",]$plant.specific.mass..g)))
sage.df$type="sage"
names(sage.df)[1]="Treatment"
names(sage.df)[2]="Tank"
names(sage.df)[3]="plant.specific.mass..g"
names(sage.df)[4]="abs.change"
names(sage.df)[5]="percent.change"
sage.df[is.na(sage.df)] = 0
sage.df$mass.remaining=100-abs(sage.df[sage.df$type=="sage",]$percent.change)

#create a willow dataframe
willow.df=data.frame(decomp.df[decomp.df$plant.type=="willow",]$treatment,
                    decomp.df[decomp.df$plant.type=="willow",]$tank,
                    decomp.df[decomp.df$plant.type=="willow",]$plant.specific.mass..g,
                    decomp.df[decomp.df$plant.type=="willow",]$dry.mass..g.bag.corrected-decomp.df[decomp.df$plant.type=="willow",]$plant.specific.mass..g,
                     (100*(decomp.df[decomp.df$plant.type=="willow",]$dry.mass..g.bag.corrected-decomp.df[decomp.df$plant.type=="willow",]$plant.specific.mass..g)/(decomp.df[decomp.df$plant.type=="willow",]$plant.specific.mass..g)))
willow.df$type="willow"
names(willow.df)[1]="Treatment"
names(willow.df)[2]="Tank"
names(willow.df)[3]="plant.specific.mass..g"
names(willow.df)[4]="abs.change"
names(willow.df)[5]="percent.change"
willow.df[is.na(willow.df)] = 0
willow.df$mass.remaining=100-abs(willow.df[willow.df$type=="willow",]$percent.change)

#bind them
plant.df=rbind(sage.df, willow.df)

#remove mass remaining > 100 since those are the 'no biomass' tanks
plant.df=plant.df[!(plant.df$mass.remaining>=100),]

#transform for analysis
decomp2.df=decomp2.df%>% 
 mutate(
    across(c(9),
           .fns = ~asin(sqrt(.))))

```



Sage and Willow Decomp Plots (% mass remaining)
```{r}
Fig.formatting.decomp<-(theme_classic()) +
  theme(text=element_text(size=14),
        legend.text.align = 0,
        panel.border = element_rect(fill=NA, colour = "black", size=1),
        aspect.ratio=1, 
        axis.ticks.length=unit(0.25, "cm"),
        axis.text.y=element_text(
        margin=unit(c(0.5, 0.5, 0.5, 0.5), "cm"), colour="black", size=12), 
        axis.text.x=element_text(
        margin=unit(c(0.5, 0.5, 0.5, 0.5), "cm"), colour="black", size=12)) +
  theme(legend.key.size = unit(0.4, "cm")) +
  theme(aspect.ratio=1.3) +
  theme(panel.spacing=unit(c(0, 0, 0, 0), "cm"))


#sage treatment decomp
mass.sage.decomp.plot=ggplot(plant.df[(plant.df$type=="sage"),], aes(x=plant.specific.mass..g, y=percent.change, color=Treatment, linetype=Treatment))+ 
  geom_point(alpha=0.4)+
   geom_smooth(method=gam, formula= y~s(x), aes(fill=Treatment), alpha=0.3) +
 scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Sage") +
  ylab(expression(paste("Change Sage (%)"))) +
  xlab("Plant specific mass (g)")+
  Fig.formatting+
  coord_cartesian(ylim = c(-80,-8))

mass.sage.decomp.abs.plot=ggplot(plant.df[(plant.df$type=="sage"),], aes(x=plant.specific.mass..g, y=abs.change, color=Treatment, linetype=Treatment))+ 
  geom_point(alpha=0.4)+
   geom_smooth(method=gam, formula= y~s(x), aes(fill=Treatment), alpha=0.3) +
 scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Sage") +
  ylab(expression(paste(Delta~"Sage mass (g)"))) +
  xlab("Plant specific mass (g)")+
  Fig.formatting
 


mass.sage.decomp.abs.plot=mass.sage.decomp.abs.plot+ labs(tag = "A")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

mass.sage.decomp.plot=mass.sage.decomp.plot+ labs(tag = "C")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")



sage.plots=plot_grid(mass.sage.decomp.abs.plot, mass.sage.decomp.plot, nrow=2)

  
#willow decomp
mass.willow.decomp.plot=ggplot(plant.df[(plant.df$type=="willow"),], aes(x=plant.specific.mass..g, y=percent.change, color=Treatment, linetype=Treatment))+ 
   geom_point(alpha=0.4)+
   geom_smooth(method=gam, formula= y~s(x), aes(fill=Treatment), alpha=0.3) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
   scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Willow") +
  ylab("Change Willow (%)") +
   xlab("Plant specific mass (g)")+
  coord_cartesian(ylim = c(-80,-8))+
  Fig.formatting

mass.willow.decomp.plot


mass.willow.decomp.abs.plot=ggplot(plant.df[(plant.df$type=="willow"),], aes(x=plant.specific.mass..g, y=abs.change, color=Treatment, linetype=Treatment))+ 
   geom_point(alpha=0.4)+
   geom_smooth(method=gam, formula= y~s(x), aes(fill=Treatment), alpha=0.3) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
   scale_fill_manual(values = c("darkred", "darkgreen")) + 
  ggtitle("Willow") +
  ylab(expression(Delta~"Willow mass (g)")) +
   xlab("Plant specific mass (g)")+
  Fig.formatting

mass.willow.decomp.abs.plot=mass.willow.decomp.abs.plot + labs(tag = "B")+
<<<<<<< HEAD
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

mass.willow.decomp.plot=mass.willow.decomp.plot + labs(tag = "D")+
=======
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

mass.willow.decomp.plot=mass.willow.decomp.plot + labs(tag = "D")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")



willow.plots=plot_grid(mass.willow.decomp.abs.plot, mass.willow.decomp.plot, nrow=2)

willow.plots=plot_grid(mass.willow.decomp.abs.plot, mass.willow.decomp.plot, nrow=2)

#combine them

mass.decomp.plots=plot_grid(
  sage.plots, willow.plots, ncol=2)



mass.decomp.plots
ggsave("figures/mass.plant_type_decomp.pdf", height=8, width=12)


```



GAM for sage/willow
```{r}
plant.gam=gam(abs.change ~ Treatment*type + s(plant.specific.mass..g, by=type), data=plant.df, method= "REML", select=T)
summary(plant.gam)
anova(plant.gam)

GAM for sage/willow
```{r}
plant.gam=gam(abs.change ~ Treatment*type + s(plant.specific.mass..g, by=type), data=plant.df, method= "REML", select=T)
summary(plant.gam)
anova(plant.gam)


plant.perc.gam=gam(percent.change ~ Treatment*type + s(plant.specific.mass..g, by=type), data=plant.df, method= "REML", select=T)
summary(plant.perc.gam)
anova(plant.perc.gam)


mean(plant.df[plant.df$type=="sage",]$abs.change)
mean(plant.df[plant.df$type=="willow",]$abs.change)
```



SUVA plots
```{r}



suva.T0<-ggplot(suva.df[(suva.df$Time.point=="T0"),], aes(x=plant.mass..g, y=SUVA, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-0") +
  xlab("")+
 ylab(expression('SUVA'["254"]~'(mgC'~ L^-1~m^-1*')'))+
  coord_cartesian(ylim = c(0,6))+
  Fig.formatting

plant.perc.gam=gam(percent.change ~ Treatment*type + s(plant.specific.mass..g, by=type), data=plant.df, method= "REML", select=T)
summary(plant.perc.gam)
anova(plant.perc.gam)

mean(plant.df[plant.df$type=="sage",]$abs.change)
mean(plant.df[plant.df$type=="willow",]$abs.change)
```



SUVA plots
```{r}




suva.T0<-ggplot(suva.df[(suva.df$Time.point=="T0"),], aes(x=plant.mass..g, y=SUVA, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-0") +
  xlab("")+
 ylab(expression('SUVA'["254"]~'(mgC'~ L^-1~m^-1*')'))+
  coord_cartesian(ylim = c(0,6))+
  Fig.formatting


suva.T1<-ggplot(suva.df[(suva.df$Time.point=="T1"),], aes(x=plant.mass..g, y=SUVA, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-1") +
  xlab("Plant mass (g)")+
ylab(expression('SUVA'["254"]~'(mgC'~ L^-1~m^-1*')'))+
  ylim(1.2,3.5)+
  Fig.formatting
suva.T1
=======
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0

suva.T1<-ggplot(suva.df[(suva.df$Time.point=="T1"),], aes(x=plant.mass..g, y=SUVA, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-1") +
<<<<<<< HEAD
  xlab("")+
   ylab(bquote(""))+
  ylim(0,6)+
  Fig.formatting

  
  
suva.T2<-ggplot(suva.df[(suva.df$Time.point=="T2"),], aes(x=plant.mass..g, y=SUVA, color=Treatment, linetype=Treatment)) +
=======
  xlab("Plant mass (g)")+
ylab(expression('SUVA'["254"]~'(mgC'~ L^-1~m^-1*')'))+
  ylim(1.2,3.5)+
  Fig.formatting
suva.T1

suva.T1<-ggplot(suva.df[(suva.df$Time.point=="T1"),], aes(x=plant.mass..g, y=SUVA, color=Treatment, linetype=Treatment)) +
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
<<<<<<< HEAD
  ggtitle("Time-2") +
  xlab("Plant mass (g)")+
  ylab(expression(""))+
    ylim(0,6)+
  Fig.formatting

suva.T2
=======
  ggtitle("Time-1") +
  xlab("")+
   ylab(bquote(""))+
  ylim(0,6)+
  Fig.formatting

  
  
suva.T2<-ggplot(suva.df[(suva.df$Time.point=="T2"),], aes(x=plant.mass..g, y=SUVA, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-2") +
  xlab("Plant mass (g)")+
  ylab(expression(""))+
    ylim(0,6)+
  Fig.formatting
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0

suva.T2


suva.T3<-ggplot(suva.df[(suva.df$Time.point=="T3"),], aes(x=plant.mass..g, y=SUVA, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-3") +
  xlab("")+
  ylab(expression(""))+
    coord_cartesian(ylim = c(0,6))+
  Fig.formatting

suva.T3<-ggplot(suva.df[(suva.df$Time.point=="T3"),], aes(x=plant.mass..g, y=SUVA, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-3") +
  xlab("")+
  ylab(expression(""))+
    coord_cartesian(ylim = c(0,6))+
  Fig.formatting

suva.T4<-ggplot(suva.df[(suva.df$Time.point=="T4"),], aes(x=plant.mass..g, y=SUVA, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-4") +
  xlab("")+
  ylab(expression(""))+
    coord_cartesian(ylim = c(0,6))+
  Fig.formatting

<<<<<<< HEAD
extract.legend5 <- get_legend(
  # create some space to the left of the legend
  suva.T0 + theme(legend.box.margin = margin(0, 0, 0, 10)))

library(ggplot2)
library(cowplot)
library(grid)
library(gridExtra)
library(ggpubr)


suva.plots<-plot_grid(
  suva.T0+ theme(legend.position = "none"),
  suva.T1+ theme(legend.position = "none"),
  suva.T2+ theme(legend.position = "none"),
  suva.T3+ theme(legend.position = "none"),
  suva.T4+ theme(legend.position = "none"),
  extract.legend5,
  rel_widths = c(50,50,50,50,50, 20), ncol=6)

suva.plots

suva.plots=suva.plots+ labs(tag = "D")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")



ggsave("figures/suva.pdf", height=5, width=17.5)



```


SR plots
```{r}

sr.T0<-ggplot(suva.df[(suva.df$Time.point=="T0"),], aes(x=plant.mass..g, y=Spectral.Slope.Ratio, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-0") +
  xlab("")+
  ylab(expression('S'[italic(R)]))+
coord_cartesian(ylim=c(1,6))+
  Fig.formatting

sr.T0
=======
suva.T4<-ggplot(suva.df[(suva.df$Time.point=="T4"),], aes(x=plant.mass..g, y=SUVA, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-4") +
  xlab("")+
  ylab(expression(""))+
    coord_cartesian(ylim = c(0,6))+
  Fig.formatting

extract.legend5 <- get_legend(
  # create some space to the left of the legend
  suva.T0 + theme(legend.box.margin = margin(0, 0, 0, 10)))

library(ggplot2)
library(cowplot)
library(grid)
library(gridExtra)
library(ggpubr)


suva.plots<-plot_grid(
  suva.T0+ theme(legend.position = "none"),
  suva.T1+ theme(legend.position = "none"),
  suva.T2+ theme(legend.position = "none"),
  suva.T3+ theme(legend.position = "none"),
  suva.T4+ theme(legend.position = "none"),
  extract.legend5,
  rel_widths = c(50,50,50,50,50, 20), ncol=6)

suva.plots

suva.plots=suva.plots+ labs(tag = "D")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")



ggsave("figures/suva.pdf", height=5, width=17.5)


>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0


<<<<<<< HEAD

sr.T1<-ggplot(suva.df[(suva.df$Time.point=="T1"),], aes(x=plant.mass..g, y=Spectral.Slope.Ratio, color=Treatment, linetype=Treatment)) +
=======

SR plots
```{r}

sr.T0<-ggplot(suva.df[(suva.df$Time.point=="T0"),], aes(x=plant.mass..g, y=Spectral.Slope.Ratio, color=Treatment, linetype=Treatment)) +
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
<<<<<<< HEAD
  ggtitle("Time-1") +
  xlab("")+
  ylab("")+
   coord_cartesian(ylim=c(0,3))+
  Fig.formatting

sr.T1

  
sr.T2<-ggplot(suva.df[(suva.df$Time.point=="T2"),], aes(x=plant.mass..g, y=Spectral.Slope.Ratio, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-2") +
  xlab("Plant mass (g)")+
  ylab(bquote(""))+
     coord_cartesian(ylim=c(0,3))+
  Fig.formatting



sr.T3<-ggplot(suva.df[(suva.df$Time.point=="T3"),], aes(x=plant.mass..g, y=Spectral.Slope.Ratio, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-3") +
  xlab("")+
  ylab(bquote(""))+
     coord_cartesian(ylim=c(0,3))+
  Fig.formatting


sr.T4<-ggplot(suva.df[(suva.df$Time.point=="T4"),], aes(x=plant.mass..g, y=Spectral.Slope.Ratio, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-4") +
  xlab("")+
  ylab(bquote(""))+
     coord_cartesian(ylim=c(0,3))+
  Fig.formatting

extract.legend5 <- get_legend(
  # create some space to the left of the legend
  sr.T0 + theme(legend.box.margin = margin(0, 0, 0, 10)))


sr.plots<-plot_grid(
  sr.T0+ theme(legend.position = "none"),
  sr.T1+ theme(legend.position = "none"),
  sr.T2+ theme(legend.position = "none"),
  sr.T3+ theme(legend.position = "none"),
  sr.T4+ theme(legend.position = "none"),
  extract.legend5,
  rel_widths = c(50,50,50,50,50, 20), ncol=6)

sr.plots

=======
  ggtitle("Time-0") +
  xlab("")+
  ylab(expression('S'[italic(R)]))+
coord_cartesian(ylim=c(1,6))+
  Fig.formatting

sr.T0



sr.T1<-ggplot(suva.df[(suva.df$Time.point=="T1"),], aes(x=plant.mass..g, y=Spectral.Slope.Ratio, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-1") +
  xlab("")+
  ylab("")+
   coord_cartesian(ylim=c(0,3))+
  Fig.formatting

sr.T1

  
sr.T2<-ggplot(suva.df[(suva.df$Time.point=="T2"),], aes(x=plant.mass..g, y=Spectral.Slope.Ratio, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-2") +
  xlab("Plant mass (g)")+
  ylab(bquote(""))+
     coord_cartesian(ylim=c(0,3))+
  Fig.formatting



sr.T3<-ggplot(suva.df[(suva.df$Time.point=="T3"),], aes(x=plant.mass..g, y=Spectral.Slope.Ratio, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-3") +
  xlab("")+
  ylab(bquote(""))+
     coord_cartesian(ylim=c(0,3))+
  Fig.formatting


sr.T4<-ggplot(suva.df[(suva.df$Time.point=="T4"),], aes(x=plant.mass..g, y=Spectral.Slope.Ratio, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-4") +
  xlab("")+
  ylab(bquote(""))+
     coord_cartesian(ylim=c(0,3))+
  Fig.formatting

extract.legend5 <- get_legend(
  # create some space to the left of the legend
  sr.T0 + theme(legend.box.margin = margin(0, 0, 0, 10)))


sr.plots<-plot_grid(
  sr.T0+ theme(legend.position = "none"),
  sr.T1+ theme(legend.position = "none"),
  sr.T2+ theme(legend.position = "none"),
  sr.T3+ theme(legend.position = "none"),
  sr.T4+ theme(legend.position = "none"),
  extract.legend5,
  rel_widths = c(50,50,50,50,50, 20), ncol=6)

sr.plots

>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
sr.plots=sr.plots+ labs(tag = "C")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")



ggsave("figures/sr.plots.pdf", height=5, width=17.5)



```

Combine UVBIO T1 with DOC T1

```{r}

UVBIO.T1.DOC.abs=UVBIO.T1.DOC.abs + labs(tag = "A")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")



DOC.uvbio.abs.plots=plot_grid(UVBIO.T1.DOC.abs, UVBIO.T1.effect.abs, nrow=2)


UVBIO.T1.DOC=UVBIO.T1.DOC +labs(tag = "C")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

DOC.uvbio.perc.plots=plot_grid(UVBIO.T1.DOC, UVBIO.T1.effect, nrow=2)


UVBIO.T1.effect.abs=UVBIO.T1.effect.abs + labs(tag = "A")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

UVBIO.T1.effect=UVBIO.T1.effect + labs(tag = "B")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")



all.DOC.uvbio.plots=plot_grid(UVBIO.T1.effect.abs, UVBIO.T1.effect, nrow=2)


ggsave("figures/T1.DOC.degra.combo.plot.pdf", height=10, width=11)


```


Combine UVBIO T1 with SUVA T1

```{r}

suva.T1=suva.T1 + labs(tag = "A")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

UVBIO.T1.DOC.suva=UVBIO.T1.DOC.suva +labs(tag = "B")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

suva.uvbio.plots=plot_grid(suva.T1, UVBIO.T1.DOC.suva, ncol=2)

UVBIO.T1.effect.suva=UVBIO.T1.effect.suva + labs(tag = "C")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")
<<<<<<< HEAD


all.suva.uvbio.plots=plot_grid(suva.uvbio.plots, UVBIO.T1.effect.suva, nrow=2)


ggsave("figures/T1.suva.degra.combo.plot.pdf", height=10, width=11)


```


Combine UVBIO T1 with HIX T1

```{r}

hix.T1=hix.T1 + labs(tag = "A")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

UVBIO.T1.DOC.hix=UVBIO.T1.DOC.hix +labs(tag = "B")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

hix.uvbio.plots=plot_grid(hix.T1, UVBIO.T1.DOC.hix, ncol=2)

UVBIO.T1.effect.hix=UVBIO.T1.effect.hix + labs(tag = "C")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")


all.hix.uvbio.plots=plot_grid(hix.uvbio.plots, UVBIO.T1.effect.hix, nrow=2)

all.hix.uvbio.plots

ggsave("figures/T1.HIX.degra.combo.plot.pdf", height=10, width=11)


```


Combine UVBIO T1 with SPEC T1

```{r}

sr.T1=sr.T1 + labs(tag = "A")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

UVBIO.T1.DOC.spec=UVBIO.T1.DOC.spec +labs(tag = "B")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

=======


all.suva.uvbio.plots=plot_grid(suva.uvbio.plots, UVBIO.T1.effect.suva, nrow=2)


ggsave("figures/T1.suva.degra.combo.plot.pdf", height=10, width=11)


```


Combine UVBIO T1 with HIX T1

```{r}

hix.T1=hix.T1 + labs(tag = "A")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

UVBIO.T1.DOC.hix=UVBIO.T1.DOC.hix +labs(tag = "B")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

hix.uvbio.plots=plot_grid(hix.T1, UVBIO.T1.DOC.hix, ncol=2)

UVBIO.T1.effect.hix=UVBIO.T1.effect.hix + labs(tag = "C")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")


all.hix.uvbio.plots=plot_grid(hix.uvbio.plots, UVBIO.T1.effect.hix, nrow=2)

all.hix.uvbio.plots

ggsave("figures/T1.HIX.degra.combo.plot.pdf", height=10, width=11)


```


Combine UVBIO T1 with SPEC T1

```{r}

sr.T1=sr.T1 + labs(tag = "A")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

UVBIO.T1.DOC.spec=UVBIO.T1.DOC.spec +labs(tag = "B")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
spec.uvbio.plots=plot_grid(sr.T1, UVBIO.T1.DOC.spec, ncol=2)

UVBIO.T1.effect.spec=UVBIO.T1.effect.spec+ labs(tag = "C")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")


all.spec.uvbio.plots=plot_grid(spec.uvbio.plots, UVBIO.T1.effect.spec, nrow=2)



ggsave("figures/T1.spec.degra.combo.plot.pdf", height=10, width=11)


```





SUVA GAM
Time point 1
```{r}

suva.gam1=gam(SUVA ~ factor(Treatment) +s(plant.mass..g, by=factor(Treatment)), data = suva.df, method="REML", select= T, subset = Time.point=="T1")
anova(suva.gam1)
summary(suva.gam1)
gam.check(suva.gam1, rep=1000)


```
Time point 2

```{r}

suva.gam2=gam(SUVA ~ factor(Treatment) +s(plant.mass..g, by=factor(Treatment)), data = suva.df, method="REML", select= T, subset = Time.point=="T2")
summary(suva.gam2)
anova(suva.gam2)
gam.check(suva.gam2, rep=1000)

```
Time point 3
```{r}
suva.gam3=gam(SUVA ~ Treatment +s(plant.mass..g, by=factor(Treatment)), data = suva.df, method="REML", select= T, subset = Time.point=="T3")
summary(suva.gam3)
anova(suva.gam3)
gam.check(suva.gam3, rep=1000)



```
Time point 4
```{r}

suva.gam4=gam(SUVA ~ factor(Treatment) +s(plant.mass..g, by=factor(Treatment)), data = suva.df, method="REML", select= T, subset = Time.point=="T4")
anova(suva.gam4)
summary(suva.gam4)
gam.check(suva.gam4, rep=1000)




```


Index correlation
```{r}
suva.vs.sr.plot=ggplot(DOC.df2[(DOC.df2$Treatment=="unburned"),], aes(x=plant.mass..g, y=Type, color=name, linetype=name)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("blue", "yellow", "green", "orange")) + 
 scale_fill_manual(values = c("blue", "yellow", "green", "orange")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=name), alpha=0.3) +
  ggtitle("Time-1") +
  xlab("SUVA")+
   ylab(bquote("SR"))+
  Fig.formatting.degra

ggplot(DOC.df[(DOC.df$Treatment=="unburned"),], aes(x=SUVA, y=fl)) +
  geom_point()+
  ggtitle("Unburned: SUVA vs FI") + 
  geom_smooth(method=lm, se=FALSE) + 
  scale_x_continuous(name = "SUVA", limits = c(0,6))+
  scale_y_continuous(name = "FI", limits = c(1, 2.5))+
  Fig.formatting

ggplot(DOC.df[(DOC.df$Treatment=="burned"),], aes(x=SUVA, y=fl)) +
  geom_point()+
  ggtitle("Unburned: SUVA vs FI") + 
  geom_smooth(method=lm, se=FALSE) + 
  scale_x_continuous(name = "SUVA", limits = c(0,6))+
  scale_y_continuous(name = "FI", limits = c(1, 2.5))+
  Fig.formatting
                     
                     
suva.vs.hix.plot=ggplot(DOC.df[(DOC.df$Treatment=="unburned"),], aes(x=SUVA, y=hix, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-1") +
  xlab("SUVA")+
   ylab(bquote("HIX"))+
  Fig.formatting
suva.vs.hix.plot



hix.vs.sr.plot=ggplot(DOC.df[(DOC.df$Time.point=="T1"),], aes(x=sr, y=hix, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-1") +
  xlab("SR")+
   ylab(bquote("HIX"))+
  Fig.formatting

hix.vs.fl.plot=ggplot(DOC.df[(DOC.df$Time.point=="T1"),], aes(x=fl, y=hix, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-1") +
  xlab("Fl")+
   ylab(bquote("HIX"))+
  xlim(1,2.5)+
  Fig.formatting
<<<<<<< HEAD
=======


ggplot(data = DOC.df2) + 
  geom_smooth(mapping = aes(x = plant.mass..g, y = Type, linetype = name))
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0


ggplot(data = DOC.df2) + 
  geom_smooth(mapping = aes(x = plant.mass..g, y = Type, linetype = name))


<<<<<<< HEAD


=======
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
devtools::install_github("eliocamp/tagger")
```


Index across time

```{r}
library(dplyr)


T1.index.plot=ggplot(DOC.df2[(DOC.df2$Time.point=="T1"),], aes(x=plant.mass..g, y=Type, color=Treatment, linetype=Treatment))+
  facet_grid(~name, labeller= label_parsed)+
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-1") +
  xlab("Plant mass (g)")+
   ylab(bquote("Index Values"))+
  Fig.formatting.degra

T1.index.plot

T1.index.plot=T1.index.plot+ labs(tag = "A")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

<<<<<<< HEAD

T2.index.plot=ggplot(DOC.df2[(DOC.df2$Time.point=="T2"),], aes(x=plant.mass..g, y=Type, color=Treatment, linetype=Treatment))+
  facet_grid(~name)+
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-2") +
  xlab("Plant mass (g)")+
   ylab(bquote("Index Values"))+
  ylim(0,6)+
  Fig.formatting.degra

T2.index.plot=T2.index.plot+ labs(tag = "B")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

=======

T2.index.plot=ggplot(DOC.df2[(DOC.df2$Time.point=="T2"),], aes(x=plant.mass..g, y=Type, color=Treatment, linetype=Treatment))+
  facet_grid(~name)+
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-2") +
  xlab("Plant mass (g)")+
   ylab(bquote("Index Values"))+
  ylim(0,6)+
  Fig.formatting.degra

T2.index.plot=T2.index.plot+ labs(tag = "B")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
T3.index.plot=ggplot(DOC.df2[(DOC.df2$Time.point=="T3"),], aes(x=plant.mass..g, y=Type, color=Treatment, linetype=Treatment))+
  facet_grid(~name)+
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-3") +
  xlab("Plant mass (g)")+
   ylab(bquote("Index Values"))+
  Fig.formatting.degra
<<<<<<< HEAD

T3.index.plot=T3.index.plot+ labs(tag = "C")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

=======

T3.index.plot=T3.index.plot+ labs(tag = "C")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
T4.index.plot=ggplot(DOC.df2[(DOC.df2$Time.point=="T4"),], aes(x=plant.mass..g, y=Type, color=Treatment, linetype=Treatment))+
  facet_grid(~name)+
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-4") +
  xlab("Plant mass (g)")+
   ylab(bquote("Index Values"))+
  Fig.formatting.degra

T4.index.plot=T4.index.plot+ labs(tag = "D")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")


    
index.combo.plots=plot_grid(
  T1.index.plot, 
  T2.index.plot,
  T3.index.plot,
  T4.index.plot,
  nrow=2, ncol=2
)

index.combo.plots
ggsave("figures/index.combo.plots.pdf", height=12, width=24)



```

fluorescence plots
```{r}
fl.T0<-ggplot(index.df[(index.df$Time.point=="T0"),], aes(x=plant.mass..g., y=Fluroscence.Index, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-0") +
  xlab("")+
   ylab(bquote("Fl"))+
  ylim(1,2.5)+
  Fig.formatting


fl.T1<-ggplot(index.df[(index.df$Time.point=="T1"),], aes(x=plant.mass..g., y=Fluroscence.Index, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-1") +
  xlab("")+
   ylab(bquote(""))+
  ylim(1,2.5)+
  Fig.formatting


fl.T2<-ggplot(index.df[(index.df$Time.point=="T2"),], aes(x=plant.mass..g., y=Fluroscence.Index, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-2") +
  xlab("Plant mass (g)")+
   ylab(bquote(""))+
  ylim(1,2.5)+
  Fig.formatting



fl.T3<-ggplot(index.df[(index.df$Time.point=="T3"),], aes(x=plant.mass..g., y=Fluroscence.Index, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-3") +
  xlab("")+
   ylab(bquote(""))+
  ylim(1,2.5)+
  Fig.formatting


fl.T4<-ggplot(index.df[(index.df$Time.point=="T4"),], aes(x=plant.mass..g., y=Fluroscence.Index, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-4") +
  xlab("")+
   ylab(bquote(""))+
  ylim(1,2.5)+
  Fig.formatting

extract.legend5 <- get_legend(
  # create some space to the left of the legend
  suva.T0 + theme(legend.box.margin = margin(0, 0, 0, 10)))


fl.plots<-plot_grid(
  fl.T0+ theme(legend.position = "none"),
  fl.T1+ theme(legend.position = "none"),
  fl.T2+ theme(legend.position = "none"),
  fl.T3+ theme(legend.position = "none"),
  fl.T4+ theme(legend.position = "none"),
  extract.legend5,
  rel_widths = c(50,50,50,50,50, 20), ncol=6)

fl.plots

fl.plots=fl.plots+ labs(tag = "A")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")

```


humic plots
```{r}


hix.T0<-ggplot(index.df[(index.df$Time.point=="T0"),], aes(x=plant.mass..g., y=Humification.Index, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-0") +
  xlab("")+
   ylab(bquote("HIX"))+
  coord_cartesian(ylim = c(0,5))+
  Fig.formatting



hix.T1<-ggplot(index.df[(index.df$Time.point=="T1"),], aes(x=plant.mass..g., y=Humification.Index, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-1") +
  xlab("")+
   ylab(bquote(""))+
    coord_cartesian(ylim = c(0,5))+
  Fig.formatting


hix.T2<-ggplot(index.df[(index.df$Time.point=="T2"),], aes(x=plant.mass..g., y=Humification.Index, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-2") +
  xlab("Plant mass (g)")+
   ylab(bquote(""))+
    coord_cartesian(ylim = c(0,5))+
  Fig.formatting



hix.T3<-ggplot(index.df[(index.df$Time.point=="T3"),], aes(x=plant.mass..g., y=Humification.Index, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-3") +
  xlab("")+
   ylab(bquote(""))+
    coord_cartesian(ylim = c(0,5))+
  Fig.formatting



hix.T4<-ggplot(index.df[(index.df$Time.point=="T4"),], aes(x=plant.mass..g., y=Humification.Index, color=Treatment, linetype=Treatment)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values = c("darkred", "darkgreen")) + 
  scale_fill_manual(values = c("darkred", "darkgreen")) + 
  geom_smooth(method=gam, formula= y~ s(x), aes(fill=Treatment), alpha=0.3) +
  ggtitle("Time-4") +
  xlab("")+
   ylab(bquote(""))+
    coord_cartesian(ylim = c(0,5))+
  Fig.formatting




extract.legend5 <- get_legend(
  # create some space to the left of the legend
  suva.T0 + theme(legend.box.margin = margin(0, 0, 0, 10)))


hix.plots<-plot_grid(
  hix.T0+ theme(legend.position = "none"),
  hix.T1+ theme(legend.position = "none"),
   hix.T2+ theme(legend.position = "none"),
  hix.T3+ theme(legend.position = "none"),
  hix.T4+ theme(legend.position = "none"),
  extract.legend5,
  rel_widths = c(50,50,50,50,50, 20), ncol=6)

hix.plots

hix.plots=hix.plots+ labs(tag = "B")+
  theme(plot.tag = element_text(face = "bold"),
        plot.tag.position = "topleft")



```


All indices plot
```{r}


index.combo.plots=plot_grid(fl.plots, hix.plots, sr.plots, suva.plots, nrow=4)


ggsave("figures/index.combo.plots.pdf", height=15, width=18)


```


Index analysis

FI index

```{r}
index.df$Treatment=as.factor(index.df$Treatment)

fi.gam.T1=gam(Fluroscence.Index~Treatment + s(plant.mass..g., by=Treatment), subset = Time.point=="T1", data=index.df, methods="REML", select=T)
summary(fi.gam.T1)
anova(fi.gam.T1)

fi.gam.T2=gam(Fluroscence.Index~Treatment + s(plant.mass..g., by=Treatment), subset = Time.point=="T2", data=index.df, methods="REML", select=T)
summary(fi.gam.T2)
anova(fi.gam.T2)


fi.gam.T3=gam(Fluroscence.Index~Treatment + s(plant.mass..g., by=Treatment), subset = Time.point=="T3", data=index.df, methods="REML", select=T)
summary(fi.gam)
anova(fi.gam)


fi.gam.T4=gam(Fluroscence.Index~Treatment + s(plant.mass..g., by=Treatment), subset = Time.point=="T4", data=index.df, methods="REML", select=T)
summary(fi.gam)
anova(fi.gam)


gam.check(fi.gam, rep=1000)

?gam.control
<<<<<<< HEAD

```

HIX
=======

```

HIX

```{r}
hix.gam.T1=gam(Humification.Index~Treatment + s(plant.mass..g., by=Treatment), subset=Time.point=="T1",data=index.df, methods="REML", select=T)
summary(hix.gam)
anova(hix.gam)
gam.check(hix.gam, rep=1000)
>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0

```{r}
hix.gam.T1=gam(Humification.Index~Treatment + s(plant.mass..g., by=Treatment), subset=Time.point=="T1",data=index.df, methods="REML", select=T)
summary(hix.gam)
anova(hix.gam)
gam.check(hix.gam, rep=1000)

<<<<<<< HEAD

hix.gam.T4=gam(Humification.Index~Treatment + s(plant.mass..g., by=Treatment), subset=Time.point=="T4",data=index.df, methods="REML", select=T)
summary(hix.gam.T4)
anova(hix.gam.T4)

mean(index.df[index.df$Time.point=="T4",]$Humification.Index)


=======
hix.gam.T4=gam(Humification.Index~Treatment + s(plant.mass..g., by=Treatment), subset=Time.point=="T4",data=index.df, methods="REML", select=T)
summary(hix.gam.T4)
anova(hix.gam.T4)

mean(index.df[index.df$Time.point=="T4",]$Humification.Index)


>>>>>>> ec586a7382aec292ba46042e721e0f8385b128a0
```
SUVA
```{r}
suva.df$Treatment=as.factor(suva.df$Treatment)
suva.gam.T1=gam(SUVA~Treatment + s(plant.mass..g, by=Treatment), subset=Time.point=="T1",data=suva.df, methods="REML", select=T)
summary(suva.gam.T1)
anova(suva.gam.T1)
gam.check(hix.gam, rep=1000)




```

Sr
```{r}

sr.gam.T1=gam(Spectral.Slope.Ratio~Treatment + s(plant.mass..g, by=Treatment), subset=Time.point=="T1",data=suva.df, methods="REML", select=T)
summary(sr.gam.T1)





```

Add new metrics to DOC


###DOC  
import and do a loop to clean up all files and make stacked data in single df
```{r}

#Combine files from loop to single df
DOC_T0$SUVA=suva.df[suva.df$Time.point=="T0",]$SUVA
DOC_T1$SUVA=suva.df[suva.df$Time.point=="T1",]$SUVA
DOC_T2$SUVA=suva.df[suva.df$Time.point=="T2",]$SUVA
DOC_T3$SUVA=suva.df[suva.df$Time.point=="T3",]$SUVA
DOC_T4$SUVA=suva.df[suva.df$Time.point=="T4",]$SUVA

DOC_T0$hix=index.df[index.df$Time.point=="T0",]$Humification.Index
DOC_T1$hix=index.df[index.df$Time.point=="T1",]$Humification.Index
DOC_T2$hix=index.df[index.df$Time.point=="T2",]$Humification.Index
DOC_T3$hix=index.df[index.df$Time.point=="T3",]$Humification.Index
DOC_T4$hix=index.df[index.df$Time.point=="T4",]$Humification.Index

DOC_T0$fl=index.df[index.df$Time.point=="T0",]$Fluroscence.Index
DOC_T1$fl=index.df[index.df$Time.point=="T1",]$Fluroscence.Index
DOC_T2$fl=index.df[index.df$Time.point=="T2",]$Fluroscence.Index
DOC_T3$fl=index.df[index.df$Time.point=="T3",]$Fluroscence.Index
DOC_T4$fl=index.df[index.df$Time.point=="T4",]$Fluroscence.Index

DOC_T0$sr=suva.df[suva.df$Time.point=="T0",]$Spectral.Slope.Ratio
DOC_T1$sr=suva.df[suva.df$Time.point=="T1",]$Spectral.Slope.Ratio
DOC_T2$sr=suva.df[suva.df$Time.point=="T2",]$Spectral.Slope.Ratio
DOC_T3$sr=suva.df[suva.df$Time.point=="T3",]$Spectral.Slope.Ratio
DOC_T4$sr=suva.df[suva.df$Time.point=="T4",]$Spectral.Slope.Ratio

DOC.df<-rbind(DOC_T0, DOC_T1, DOC_T2, DOC_T3, DOC_T4)

#Extract file
DOC.df$File=sapply(strsplit(DOC.df$File, "/"), '[', 9)

# alternative way to code the above
#give the 10th-24th character of the file name, removing the rest
#DOC.df$File<-substr(DOC.df$File, 13, 27) 

#alternatively
# remove the 9 letters ('^.) at start 
# remove the 4 letters (.$') at end
#DOC.df$File<-gsub('^.........|....$', '', DOC.df$File) 

# if equals DOC_T0 then, T0, if not then T1..etc
DOC.df$Time.point<- as.factor(ifelse(DOC.df$File=="DOC_T0.csv", "T0",
         ifelse (DOC.df$File=="DOC_T1.csv", "T1",
           ifelse (DOC.df$File=="DOC_T2.csv", "T2", 
                   ifelse(DOC.df$File=="DOC_T3.csv", "T3", "T4")))))

#rearrange
library(dplyr)
DOC.df<- DOC.df %>% 
  dplyr::select(File, Time.point, Treatment, Tank, plant.mass..g, DOC..mg.L, TN..mg.L, SUVA, sr, hix, fl) 

DOC.df2=data.frame(DOC.df)

names(DOC.df2)[9]="SR"
names(DOC.df2)[10]="HIX"
names(DOC.df2)[11]="FI"

DOC.df2=pivot_longer(DOC.df2, cols = c("SUVA", "SR","HIX","FI"), values_to="Type")




```




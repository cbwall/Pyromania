...
# NEC = dusk - dawn (PM to AM, O2 change of day 1)
...
T2.Prod<-YSI.T2[(YSI.T2$Date == "2021-12-05"),] # dawn and dusk for 12h period
T2.Dawn1<-T2.Prod[(T2.Prod$Dawn..Dusk == "dawn"),] # dawn-1 measurements
T2.Dusk<-T2.Prod[(T2.Prod$Dawn..Dusk == "dusk"),] # dusk measurements

T2.Dawn2<-YSI.T2[(YSI.T2$Date == "2021-12-06"),] # dawn-2 measurements, following AM
...
#Time 1 NEC
NEC.T1<-ggplot(T1.O2, aes(x=plant.mass..g, y=NEC, color=Treatment)) + 
  geom_point() +
  scale_color_manual(values = c("brown1", "mediumseagreen")) + 
  geom_smooth(method=lm, aes(fill=Treatment), alpha=0.1) +
  geom_hline(yintercept=0, linetype="longdash", color = "gray") +
...
O2.change.T1<-plot_grid(
  NEP.T1+ theme(legend.position = "none"), 
  NEC.T1+ theme(legend.position = "none"),
  extract.legend, 
...
#Time 1 NEC
NEC.T2<-ggplot(T2.O2, aes(x=plant.mass..g, y=NEC, color=Treatment)) + 
  geom_point() +
  scale_color_manual(values = c("brown1", "mediumseagreen")) + 
  geom_smooth(method=lm, aes(fill=Treatment),alpha=0.1) +
  geom_hline(yintercept=0, linetype="longdash", color = "gray") +
...
O2.change.T2<-plot_grid(
  NEP.T2+ theme(legend.position = "none"), 
  NEC.T2+ theme(legend.position = "none"),
  extract.legend, 
  rel_widths = c(8,8,3), ncol=3)

...
# value of consumer/pool - food source 1 [i.e., no-label food]/ (food 2 [i.e.,plants]- source 1)
# using d15N values of 11 for algae/plankton stock 
# using d15N value of 297 for % sage

topes.trt$percent.sage<-(topes.trt$d15N-11)/(297-11) 

# unicode text for micrometer = \u03BC, use this in legend

iso.mix<-ggplot(topes.trt, aes(x=plant.mass..g, y=100*percent.sage, color=Treatment)) +
...
# model with biomass and treatment
###### only POM
mod1a <- gam(percent.sage ~ s(plant.mass..g, k=5, bs="tp") +
            s(plant.mass..g, Treatment, k=5, bs="re"),
            data=topes.trt[(topes.trt$Type=="POM"),], method="REML", family="gaussian")
# model with biomass
mod2a <- gam(percent.sage ~ s(plant.mass..g, k=5, bs="tp"), 
            data=topes.trt[(topes.trt$Type=="POM"),], method="REML", family="gaussian")
...
##############
## only plankton
mod1b <- gam(percent.sage ~ s(plant.mass..g, k=5, bs="tp") +
            s(plant.mass..g, Treatment, k=5, bs="re"),
            data=topes.trt[(topes.trt$Type=="plankton"),], method="REML", family="gaussian")
# model with biomass
mod2b <- gam(percent.sage ~ s(plant.mass..g, k=5, bs="tp"), 
            data=topes.trt[(topes.trt$Type=="plankton"),], method="REML", family="gaussian")

...
Generate dataframes for NEP and NER change in O2
...
m3.T1.CN <- gam(C.N ~ Treatment + Type +
            s(plant.mass..g), 
            subset = Time.point=="T1", data=topes.trt, method="REML", family="gaussian")
...
AIC.CN.T1<-AIC(m1.T1.CN, m2.T1.CN, m3.T1.CN, m4.T1.CN)
## additive model best fit, but no treatment or type effect

summary(m2.T1.CN)
anova.gam(m2.T1.CN)
gam.check(m2.T1.CN, rep=1000)
draw(m2.T1.CN)
concrvity(m2.T1.CN)
par(mfrow = c(1, 2))
plot(m2.T1.CN, all.terms = TRUE, page=1)
...
m3.T2.CN <- gam(C.N ~ Treatment + Type +
            s(plant.mass..g), 
            subset = Time.point=="T2", data=topes.trt, method="REML", family="gaussian")
...
AIC.CN.T2<-AIC(m1.T2.CN, m2.T2.CN, m3.T2.CN, m4.T2.CN)
## additive model best fit, but no treatment or type effect
...
concrvity(m2.T2.CN)
par(mfrow = c(1, 2))
plot(m2.T2.CN, all.terms = TRUE, page=1)
...
mod.CN<-rep(c("Treatment * Type + s(plant.mass..g, by=Treatment)", 
              "Treatment + Type + s(plant.mass..g, by=Treatment)", 
...
m3.T1.sage <- gam(percent.sage ~ Treatment + Type +
            s(plant.mass..g), 
            subset = Time.point=="T1", data=topes.trt, method="REML", family="gaussian")

m4.T1.sage <- gam(percent.sage ~
                     s(plant.mass..g), 
            subset = Time.point=="T1", data=topes.trt, method="REML", family="gaussian")


AIC.sage.T1<-AIC(m1.T1.sage, m2.T1.sage, m3.T1.sage, m4.T1.sage)
## additive model best fit

summary(m2.T1.sage)
anova.gam(m2.T1.sage)
gam.check(m2.T1.sage, rep=1000)
draw(m2.T1.sage)
concrvity(m2.T1.sage)
par(mfrow = c(1, 2))
plot(m2.T1.sage, all.terms = TRUE, page=1)
...
m3.T2.sage <- gam(percent.sage ~ Treatment + Type +
            s(plant.mass..g), 
            subset = Time.point=="T2", data=topes.trt, method="REML", family="gaussian")

m4.T2.sage <- gam(percent.sage ~
                     s(plant.mass..g), 
            subset = Time.point=="T2", data=topes.trt, method="REML", family="gaussian")


AIC.sage.T2<-AIC(m1.T2.sage, m2.T2.sage, m3.T2.sage, m4.T2.sage)
# m2.T2.sage preferred
# compare across

summary(m2.T2.sage)
anova.gam(m2.T2.sage)
gam.check(m2.T2.sage, rep=1000)
draw(m2.T2.sage)
concrvity(m2.T2.sage)
par(mfrow = c(1, 2))
plot(m2.T2.sage, all.terms = TRUE, page=1)
...
m3.T1.d15N <- gam(d15N ~ Treatment + Type +
            s(plant.mass..g), 
            subset = Time.point=="T1", data=topes.trt, method="REML", family="gaussian")
...
AIC.d15N.T1<-AIC(m1.T1.d15N, m2.T1.d15N, m3.T1.d15N, m4.T1.d15N)
## additive model best fit

summary(m2.T1.d15N)
anova.gam(m2.T1.d15N)
gam.check(m2.T1.d15N, rep=1000)
draw(m2.T1.d15N)
concrvity(m2.T1.d15N)
par(mfrow = c(1, 2))
plot(m2.T1.d15N, all.terms = TRUE, page=1)
...
m3.T2.d15N <- gam(d15N ~ Treatment + Type +
            s(plant.mass..g), 
            subset = Time.point=="T2", data=topes.trt, method="REML", family="gaussian")
...
AIC.d15N.T2<-AIC(m1.T2.d15N, m2.T2.d15N, m3.T2.d15N, m4.T2.d15N)
## additive model best fit

summary(m2.T2.d15N)
anova.gam(m2.T2.d15N)
gam.check(m2.T2.d15N, rep=1000)
draw(m2.T2.d15N)
concrvity(m2.T2.d15N)
par(mfrow = c(1, 2))
plot(m2.T2.d15N, all.terms = TRUE, page=1)
...
posthoc<-emmeans(plant.ZN, ~treatment| type)
multcomp::cld(posthoc, Letters=letters)
```

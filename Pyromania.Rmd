...
######## T1 model
m1.TN.gam=gam(TN..mg.L ~ Treatment + s(plant.mass..g, by=Treatment), 
...
######## T2 model
m2.TN.gam=gam(TN..mg.L ~ Treatment + s(plant.mass..g, by=Treatment), 
...
# smother significant for unburned treatments


######## T3 model
m3.TN.gam=gam(TN..mg.L ~ Treatment + s(plant.mass..g, by=Treatment), 
...
######## T4 model
m4.TN.gam=gam(TN..mg.L ~ Treatment + s(plant.mass..g, by=Treatment), 
...
# no effect of treatment, but type important
# smoother significant for both 
```
...
######## test species differences
wilcox.test(d15N~Plant, data=topes.controls.plants) # d15N sage and willow differ
wilcox.test(C.N~Plant, data=topes.controls.plants) # C.N sage and willow differ
par(mfrow=c(1,2))
boxplot(d15N~Plant, data=topes.controls.plants)
boxplot(C.N~Plant, data=topes.controls.plants)


######## test difference WITHIN willow and plankton 
wilcox.test(d15N~Plant, data=topes.controls.non.enrich) # d15N plankton and willow differ
wilcox.test(C.N~Plant, data=topes.controls.non.enrich) # C:N plankton and willow differ
par(mfrow=c(1,2))
...
mod<-lm(d15N~Burn.Trt, data=topes.controls.sage)
anova(mod) 
# no difference in d15N for burned, unburned, very bured sage

#### Sage C.N
mod<-lm(C.N~Burn.Trt, data=topes.controls.sage)
anova(mod) 
...
anova(mod) 
# Willow: no difference in unburned, burned C:N

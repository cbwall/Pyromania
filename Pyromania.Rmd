...
### Isotopes
```{r, isotopes treatment}
topes<-read.csv("data/Isotopes/Pyro_isotopes_T1.csv")
...
```{r, isotopes treatment T2}
topes2<-read.csv("data/Isotopes/Pyro_isotopes_T2.csv")
topes2$C.N <-(topes2$Total.C..ug/12)/(topes2$Total.N..ug/14) # C mol : N mol

cols2<-c("Time.point", "Treatment", "Type", "Tank") # columns to make factors
topes2[cols2] <- lapply(topes2[cols2], factor) # make all these factors

topes2.trt<-topes2[(topes2$Treatment=="burned" | topes2$Treatment=="unburned"),]
topes2.controls<-topes2[!(topes2$Treatment=="burned" | topes2$Treatment=="unburned"),]

#### isotope plots for treatments
iso.plot2<-ggplot(topes2.trt, 
                 aes(x=plant.mass..g, y=d15N, color=Treatment, shape=Type, linetype=Type)) +
  geom_point() +
  scale_color_manual(values = c("brown1", "mediumseagreen")) + 
  geom_smooth(method=loess, aes(fill=Treatment), alpha=0.1) +
...
iso.plot2
ggsave("figures/iso.plot.T2.pdf", encod="MacRoman", height=5, width=6)
```
...
d15N.cont<-aggregate(d15N~ Type, topes.controls, mean)
d15N.SD<-aggregate(d15N~ Type, topes.controls, sd)

C.N.cont<-aggregate(C.N~ Type, topes.controls, mean)
C.N.SD<-aggregate(C.N~ Type, topes.controls, sd)

iso.cont.d15N<-cbind(d15N.cont, d15N.SD[2])
colnames(iso.cont.d15N)<-c("Type", "d15N", "SD")

iso.cont.CN<-cbind(C.N.cont, C.N.SD[2])
colnames(iso.cont.CN)<-c("Type", "C.N", "SD")
...
#### controls d15N
iso.plot.control.d15N<-ggplot(data=iso.cont.d15N,  aes(x=Type, y=d15N, fill=Type)) +
...
###### control C:N
iso.plot.control.CN<-ggplot(data=iso.cont.CN,  aes(x=Type, y=C.N, fill=Type)) +
  geom_bar(stat="identity") +
  geom_errorbar(aes(ymin=C.N-SD, ymax=C.N+SD),size=.5, width=0, position=pd)+
  ylab("C:N") +
...
## combine 
control.iso<- plot_grid(iso.plot.control.d15N + theme(legend.position = "none"), 
                    iso.plot.control.CN + theme(legend.position = "none"),
                    extract.legend3, rel_widths = c(8,8,3), ncol=3)
...
d15N.cont # summary mean
sage.mean<-d15N.cont[c(3:6),]
mean(sage.mean$d15N) # sage ~297, willow 13

F.cont<-aggregate(at.P..15N  ~ Type, topes.controls, mean)
sage.meanF<-F.cont[c(3:6),]
mean(sage.meanF$at.P..15N) # sage ~0.475, willow 0.371
...
#  d15N value of base 1 = 297 permil for sage

topes.trt$percent.sage<-(topes.trt$d15N-11)/(297-11)
topes2.trt$percent.sage<-(topes2.trt$d15N-11)/(297-11) 

# unicode text for micrometer = \u03BC, use this in legend

iso.mix2<-ggplot(topes.trt, aes(x=plant.mass..g, y=100*percent.sage, color=Treatment)) +
...
iso.mix
ggsave("figures/iso.mix.pdf", encod="MacRoman", height=5, width=6)


######################

iso.mix2<-ggplot(topes2.trt, aes(x=plant.mass..g, y=100*percent.sage, color=Treatment)) +
  geom_point(aes(shape=Type)) +
  scale_shape_manual(name="Plankton", values = c(17, 16), 
                     labels = c(expression(paste("> 63"~mu,"m")), 
                                expression(paste("< 63"~mu,"m")))) +
...
iso.mix2
ggsave("figures/iso.mixT2.pdf", encod="MacRoman", height=5, width=6)

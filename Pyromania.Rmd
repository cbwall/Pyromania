...
######## T1 model
m1.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, by= Treatment), subset = Time.point=="T1", data = DOC.df, method = "REML")
...
########## T2
m2.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, by=Treatment), subset = Time.point=="T2", data = DOC.df, method = "REML")
...
########## T4
m4.gam=gam(DOC..mg.L ~ Treatment + s(plant.mass..g, by=Treatment), subset = Time.point=="T4", data = DOC.df, method = "REML")
...
######## T1 model
m1.TN.gam=gam(TN..mg.L ~ Treatment + s(plant.mass..g, by=Treatment), subset = Time.point=="T1", data = DOC.df, method = "REML")
...
concrvity(m1.TN.gam)
par(mfrow = c(1, 2))
plot(m1.TN.gam, all.terms = TRUE, page=1)

# TN smoother significant for unburned but not burned
# TN not significant by Treatment 



######## T2 model
m2.TN.gam=gam(log10(TN..mg.L) ~ Treatment + s(plant.mass..g,by=Treatment, k=4, bs="ps"), subset = Time.point=="T2", data = DOC.df, method = "REML")
...
concrvity(m2.TN.gam)
par(mfrow = c(1, 2))
plot(m2.TN.gam, all.terms = TRUE, page=1)

# no treatment effect for TN, but a tendency to be higher in unburned
# smother significant for both treatments


######## T3 model
m3.TN.gam=gam(log10(TN..mg.L) ~ Treatment + s(plant.mass..g,by=Treatment, k=4, bs="ps"), subset = Time.point=="T3", data = DOC.df, method = "REML")
...
concrvity(m3.TN.gam)
par(mfrow = c(1, 2))
plot(m3.TN.gam, all.terms = TRUE, page=1)

# no treatment effect
# significance for both smoother terms


######## T4 model
m4.TN.gam=gam(log10(TN..mg.L) ~ Treatment + s(plant.mass..g,by=Treatment, k=4, bs="ps"), subset = Time.point=="T4", data = DOC.df, method = "REML")
...
concrvity(m4.TN.gam)
par(mfrow = c(1, 2))
plot(m4.TN.gam, all.terms = TRUE, page=1)

# effect of treatment (higher TN in the burned)
# significant smoother effect for burned treatment
...
m1b.T1.sage <- gam(percent.sage ~ Treatment +
            s(plant.mass..g, by=Treatment) + 
            s(plant.mass..g, by=Type),
            subset = Time.point=="T1", data=topes.trt, method="REML", family="gaussian")
...
summary(m1a.T1.sage)
anova.gam(m1a.T1.sage)
gam.check(m1a.T1.sage, rep=1000)
draw(m1a.T1.sage)
concrvity(m1a.T1.sage)
par(mfrow = c(1, 2))
plot(m1a.T1.sage, all.terms = TRUE, page=1)


############# all plankton T2
m2a.T2.sage <- gam(percent.sage ~ Treatment + Type +
            s(plant.mass..g, by=Treatment) +
            s(plant.mass..g, by=Type),
            subset = Time.point=="T2",
            data=topes.trt, method="REML", family="gaussian")

m2b.T2.sage <- gam(percent.sage ~ Treatment +
            s(plant.mass..g, by=Treatment) + 
            s(plant.mass..g, by=Type),
            subset = Time.point=="T2",
            data=topes.trt, method="REML", family="gaussian")

m2c.T2.sage <- gam(percent.sage ~ Treatment + Type + 
              s(plant.mass..g, by=Treatment),
              subset = Time.point=="T2",
            data=topes.trt, method="REML", family="gaussian")


AIC(m2a.T2.sage, m2b.T2.sage, m2c.T2.sage) 
# m2a.T2.sage preferred
# compare across

summary(m2a.T2.sage)
anova.gam(m2a.T2.sage)
gam.check(m2a.T2.sage, rep=1000)
draw(m2a.T2.sage)
concrvity(m2a.T2.sage)
par(mfrow = c(1, 2))
plot(m2a.T2.sage, all.terms = TRUE, page=1)

...
Ktest<- read.csv("data/Ktest.csv")

#Make "Group" column into factor
Ktest$Group <- as.factor(as.character(Ktest$Group))
class(Ktest$Group)
...
#Compute stats by groups
group_by(Ktest, Group) %>%
    dplyr::summarize(count = n(),
    mean = mean(DOC, na.rm = TRUE),
    sd = sd(DOC, na.rm = TRUE),
    median = median(DOC, na.rm = TRUE),
    IQR = IQR(DOC, na.rm = TRUE)
  )

#Visualize with box plots
Ktest.box=ggboxplot(Ktest, x = "Group", y = "DOC", 
          color = "Group", palette = c("#f5f242", "#f5d142", "#82b6ed", "#3f8b9e"),
          order = c("UV/F", "UV/NF", "NU/F","NU/NF"),
          ylab = "DOC (mg/L)", xlab = "Treatment")
...
Ktest.mean=ggline(Ktest, x = "Group", y = "DOC", 
       add = c("mean_se", "jitter"), 
       order = c("UV/F", "UV/NF", "NU/F", "NU/NF"),
       ylab = "DOC (mg/L)", xlab = "Treatment")


Ktest.plots <- plot_grid(
 Ktest.box, Ktest.mean,
  rel_widths = c(8,8,8,8,3), ncol=2)

ggsave("figures/Ktest.plots.pdf", height=4, width=11)

###Perform Ktest
kruskal.test(DOC ~ Group, data = Ktest)

#Multiple pairwise-comparison between groups
pairwise.wilcox.test(Ktest$DOC, Ktest$Group,
                 p.adjust.method = "BH")

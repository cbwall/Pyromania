...
AIC.TN.mod<-cbind(mod.column, AIC.TN)
...
mod.rep3<-rep(c("~Treatment + s(plant.mass..g, by= Treatment)", 
              "~Treatment + s(plant.mass..g)", 
              "~s(plant.mass..g)"), times=3)

mod.NERNEP.df<- data.frame(mod.rep3)
...
AIC.NER.mod<-cbind(mod.column3, AIC.NER)
...
AIC.CN.T1<-AIC(m1a.T1.CN, m1b.T1.CN, m1c.T1.CN, m1d.T1.CN)
...
AIC.CN.T2<-AIC(m1a.T2.CN, m1b.T2.CN, m1c.T2.CN, m1d.T2.CN)
## additive model best fit, but no treatment or type effect
...
m1d.T1.sage <- gam(percent.sage ~
                     s(plant.mass..g), 
            subset = Time.point=="T1", data=topes.trt, method="REML", family="gaussian")


AIC.sage.T1<-AIC(m1a.T1.sage, m1b.T1.sage, m1c.T1.sage, m1d.T1.sage)
...
m1d.T2.sage <- gam(percent.sage ~
                     s(plant.mass..g), 
            subset = Time.point=="T2", data=topes.trt, method="REML", family="gaussian")


AIC.sage.T2<-AIC(m1a.T2.sage, m1b.T2.sage, m1c.T2.sage, m1d.T2.sage)
...
m1d.T1.d15N <- gam(d15N ~
            s(plant.mass..g), 
            subset = Time.point=="T1", data=topes.trt, method="REML", family="gaussian")

AIC.d15N.T1<-AIC(m1a.T1.d15N, m1b.T1.d15N, m1c.T1.d15N, m1d.T1.d15N)
## additive model best fit
...
m1d.T2.d15N <- gam(d15N ~
            s(plant.mass..g), 
            subset = Time.point=="T2", data=topes.trt, method="REML", family="gaussian")


AIC.d15N.T2<-AIC(m1a.T2.d15N, m1b.T2.d15N, m1c.T2.d15N, m1d.T2.d15N)
## additive model best fit
...
mod.column2<- data.frame(mod.CN)
AIC.d15N<-bind_rows(AIC.d15N.T1, AIC.d15N.T2)

AIC.d15N.mod<-cbind(mod.d15N, AIC.d15N)
...
m1C.T2.phos<- gam(TP.umol..l ~
            s(plant.mass..g), data=phos, method="REML", family="gaussian")

AIC.T2.phos<-AIC(m1a.T2.phos,m1b.T2.phos, m1C.T2.phos)
# factor smooth best
...
mod.column.phos<- data.frame(mod.phos)

AIC.phos.mod<-cbind(mod.column.phos, AIC.T2.phos)

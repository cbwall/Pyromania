...
mean(sage.mean$d15N) # sage ~300, willow 12

F.cont<-aggregate(at.P..15N  ~ Type, topes.controls, mean)
sage.meanF<-F.cont[c(5:8),]
mean(sage.meanF$at.P..15N) # sage ~0.422, willow 0.371

# 2 source mixing model 

# value of consumer/pool - food source 1 [i.e., no-label food]/ (food 2 [i.e.,plants]- source 1)
# using d15N values of 12 for algae/plankton stock 
# or 156 for the pooled (0.5*d15N-willow[12])+(0.5*d15N-sage[300]) 

topes.trt$percent.plant<-(topes.trt$d15N-12)/(156-12) 

iso.mix<-ggplot(topes.trt, 
                 aes(x=plant.mass..g, y=percent.plant, color=Treatment, shape=Type, linetype=Type)) +
  geom_point() +
  scale_color_manual(values = c("brown1", "mediumseagreen")) + 
  geom_smooth(method=loess, aes(fill=Treatment), alpha=0.1) +
  ylab("%plant source")+

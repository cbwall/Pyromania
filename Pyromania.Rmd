...
UVBIO.df$Tank[UVBIO.df$Tank == "NO UV F1" | 
                ...
                UVBIO.df$Tank == "UV F1" |
                UVBIO.df$Tank == "1NoUVF" |
                UVBIO.df$Tank == "1NoUVNF" |
...
UVBIO.df$Time.point<- as.factor(ifelse(UVBIO.df$File=="DOC_UV_T1", "T1",
  ifelse(UVBIO.df$File=="DOC_NoUV_T1", "T1",
          ifelse(UVBIO.df$File=="DOC_UV_T2", "T2",
                ifelse(UVBIO.df$File=="DOC_NoUV_T2", "T2", "T3")))))

#rearrange
UVBIO.df<- UVBIO.df %>% 
  select(File, Time.point, Treatment, Filter.Trt, UV.Trt, Tank, plant.mass..g, DOC..mg.L, TN..mg.L)

#create new start.DOC column
...
UVBIO.df2$start.DOC<- DOC.df$DOC..mg.L[match(UVBIO.df2$Time.point, DOC.df$Time.point)]


merge(DOC.df, UVBIO.df2, by = c("Time.point")) %>% 
  mutate(start.DOC = DOC.df$DOC..mg.L) %>%
  arrange(Time.point)

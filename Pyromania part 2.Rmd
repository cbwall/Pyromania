...
plant.nut$int.fac<-interaction(plant.nut$plant, plant.nut$type)

plant.N<-lm(N~type*plant*treatment, data=plant.nut)
anova(plant.N) # 3 way interaction

plant.P<-lm(P~type+plant+treatment, data=plant.nut)
anova(plant.P) # main effects only

plant.K<-lm(K~type+plant+treatment, data=plant.nut)
anova(plant.K) # main effects only

plant.S<-lm(S~type*plant +treatment, data=plant.nut)
anova(plant.S) # type:plant, main effects

plant.ZN<-lm(N~type*plant*treatment, data=plant.nut)
...
N.plot<- ggplot(plant.nut, aes(x=int.fac, y=N, fill=treatment)) +
  geom_boxplot() +
  scale_fill_manual(values = c("coral", "darkseagreen3")) +
...
T2.phos<-ggplot(phos, aes(x=plant.mass..g, y=TP.umol..l, color=Treatment)) + 
  geom_point() +
  scale_color_manual(values = c("brown1", "mediumseagreen")) + 
  geom_smooth(method=loess, aes(fill=Treatment), alpha=0.1) +
  ggtitle("T2.Phosphorus") +
  coord_cartesian(ylim=c(0, 50)) +
  ylab(expression(paste("Total Phosphorus ","(umol/l)"))) +
  xlab("plant material (g)") +
  Fig.formatting

T2.phos
ggsave("figures/water.phosp.pdf", height=5, width=7)
